(function()
{
    return function()
    {
        ...
        { 
        	...
        	//운영접속일자 제한
        // 	if(this.preDate < "20151026" && application.gv_runMode2 == "S")
        // 	{	
        // 		this.btnLogin.set_enable(false);
        // 		application.goMain.gfn_Msg("exit", "Information", "모바일 사용은 추후 공지일로 부터 가능합니다.", "OK", true, this);	
        // 		return;						
        // 	}	
        	
        	if (!application.gfn_IsNull(application.gv_ID))
        	{
        		this.edtId.set_value(application.gv_ID);
        	}
        	
        	if (!application.gfn_IsNull(application.gv_PW))
        	{
        	...
        	{
        		this.chk_saveID00.set_value(true);
        ...
        ********************************************************************************/
        ...
        {
        	application.gv_autoLogin = this.chk_saveID.isChecked();
        	application.gv_saveId = this.chk_saveID00.isChecked();
