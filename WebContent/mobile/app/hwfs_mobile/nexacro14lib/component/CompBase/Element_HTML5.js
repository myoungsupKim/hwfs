...
if (nexacro.Browser != "Runtime") {
	if (!nexacro.Element) {
		...
		_pTextAreaElement.setElementSize = function (width, height) {
			if (this.width != width || this.height != height) {
				var target_arr = nexacro._get_invisible_obj(this.parent.parent);
				...
				if (_doc._nexacro_preTouchInfo) {
					...
					if (dir[0] == "y") {
						...
						if (bScroll) {
							...
							nexacro._syshandler_ontouchstart(window.nexacro_HTMLSysEvent, ct, _doc._nexacro_preTouchInfo[4]);
		...
		var _iframe_eventhandler_ontouchstart = function (evt) {
			...
			if (target) {
				...
				if (scrollTop == 0 || (scrollHeight - clientHeight) <= scrollTop) {
					...
					_doc._nexacro_preTouchInfo = [clientX, clientY, screenX, screenY, evt];
				}
			}
		};
