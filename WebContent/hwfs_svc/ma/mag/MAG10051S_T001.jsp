...
<%
	...
	try	{
	    ...
	    String str_in_FICS_YN      = nullToBlank(ds_cond.getString(0, "FICS_YN"));
		...
		String str_in_CLOSE_CHK     = nullToBlank(ds_cond.getString(0, "CLOSE_CHK"));
		...
		//내역조회
		sbSql.append("    /* MAG10051S_T001.jsp */ \n");

	    if(str_in_MU_CD.equals("2005")){
	    	sbSql.append("    SELECT '999' AS SAUP_TYPE, \n");
			sbSql.append("           '기타' AS SAUP_TYPENM, \n");
	    } else {
		...
		for ( int i = 1 ; i <= 31 ; i++ ){
	        ...
	        str_DateSQL += "  DECODE((SELECT OPEN_YN FROM SLA_UPJANG_SALS_DAY WHERE UPJANG_CD = V.UPJANG_CD AND (FROM_DT <= '" + str_in_OCCUR_DATE + str_Date +"' AND TO_DT >= '"+str_in_OCCUR_DATE + str_Date +"')),NULL,'Y','N') OPEN_YN" + str_Date  + ",\n";
	        str_DateSQL += " SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', QTY, 0)) QTY" + str_Date  + ",\n";
	        str_DateSQL += " ROUND(SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', F_AMT, 0))/" + str_in_UNIT + ") F_AMT" + str_Date  + ",\n";
	        str_DateSQL += " ROUND(SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', S_AMT, 0))/" + str_in_UNIT + ") S_AMT" + str_Date  + ",\n";
	        str_DateSQL += " ROUND(SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', E_AMT, 0))/" + str_in_UNIT + ") E_AMT" + str_Date  + ",\n";
	        str_DateSQL += " ROUND((SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', F_AMT, 0)) + SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', S_AMT, 0)) + SUM(DECODE(SUBSTR(V.OCCUR_DATE, 7, 2), '" + str_Date + "', E_AMT, 0)))/" + str_in_UNIT + ") TOT_AMT" + str_Date  + ",\n";
		...
		sbSql.append("     (SELECT COUNT(D.DAY) FROM (SELECT C.UPJANG, C.DAY, DECODE((SELECT OPEN_YN FROM SLA_UPJANG_SALS_DAY WHERE UPJANG_CD =C.UPJANG AND FROM_DT <= C.DAY AND TO_DT >= C.DAY),NULL,'Y','N') AS OPEN_YN FROM (SELECT A.UPJANG, (B.CD_SY||B.CD_SM||B.CD_SD) AS DAY FROM SCO_UPJANG_MST_V A, SCC_CALENDA_DATA B) C )D WHERE D.UPJANG = V.UPJANG_CD AND D.DAY LIKE '" + str_in_OCCUR_DATE + "%' AND D.OPEN_YN = 'N') OPEN_TOT, \n");
	  ...
	  //sbSql.append("                   NVL(SUM(DECODE(T2.SALS_CLASS,'1103', T2.SAL_QUANTITY,'1104', T2.SAL_QUANTITY, 0)),0) QTY, \n");
		if(str_in_MU_CD.equals("2002")){
			...
			sbSql.append("                          NVL(SUM(CASE WHEN SALS_CLASS IN ('1204','1205','2101','2102') THEN T2.NET_AMT ELSE 0 END), 0) F_AMT, \n");
			//sbSql.append("                   NVL(DECODE(T2.VAT_CLASS,'10', \n");
			//sbSql.append("                          SUM((CASE WHEN SALS_CLASS IN ('1201','1202','1203') THEN T2.NET_AMT ELSE 0 END)), \n");
			sbSql.append("                          NVL(SUM(CASE WHEN SALS_CLASS IN ('1201','1202','1203','2104','1209') THEN T2.NET_AMT ELSE 0 END), 0) S_AMT, \n");
			//sbSql.append("                   NVL(DECODE(T2.VAT_CLASS,'10', \n");
			//sbSql.append("                          SUM((CASE WHEN SALS_CLASS IN ('1206','2103') THEN T2.NET_AMT ELSE 0 END)), \n");
			sbSql.append("                          NVL(SUM(CASE WHEN SALS_CLASS IN ('1206','1207','1208','2103','1901') THEN T2.NET_AMT ELSE 0 END), 0) E_AMT, \n");
		...
		}else if(str_in_MU_CD.equals("2005")){	 
            sbSql.append("                   NVL(SUM(T2.SAL_QUANTITY),0) AS QTY, \n");
			sbSql.append("                   0 AS F_AMT, \n");
			sbSql.append("                   NVL(DECODE(T2.VAT_CLASS,'10', \n");
			sbSql.append("                          SUM((CASE WHEN SALS_CLASS IN ('1701','2303') THEN T2.NET_AMT ELSE 0 END)), \n");
			sbSql.append("                          SUM((CASE WHEN SALS_CLASS IN ('1701','2303') THEN T2.SAL_AMT ELSE 0 END)) ), 0) S_AMT, \n");
			sbSql.append("                          NVL(SUM(CASE WHEN SALS_CLASS IN ('1801','1901','2301','2401') THEN T2.NET_AMT ELSE 0 END), 0) E_AMT, \n");
		...
		//삭제상태 데이타는 안보이도록 처리 20190221 김호석
		sbSql.append("            AND    T1.RECD_STATUS = 'L' \n");
		sbSql.append("            AND    T2.RECD_STATUS = 'L' \n");
		...
		if(str_in_MU_CD.equals("2002")){
			...
			sbSql.append("            AND    T2.SYS_CLASS IN (17,18,19,23,24,25) \n");		
		}
		if(str_in_CLOSE_CHK.equals("N")){
		...
		sbSql.append("            GROUP BY T3.SAUP_TYPE,T3.MU_CD,T3.TM_CD,T3.TM_NM,T2.UPJANG_CD,T3.UPJANGNM,T2.OCCUR_DATE,T2.VAT_CLASS \n");
...
/* 		sbSql.append("    UNION ALL \n");				
		...
		sbSql.append("                         ELSE '미지정' END) NEW_GUBUN, \n");
		sbSql.append("                   V.IF_DATE TRX_DATE, \n");
		...
		sbSql.append("            AND   (V.IF_DATE   BETWEEN '" + str_in_TRX_DATE + "'||'01' AND '" + str_in_TRX_DATE + "'||'31') \n");
		...
		sbSql.append("            GROUP BY C.SAUP_TYPE, C.TM_CD, C.UPJANG, V.TAX_CODE, V.IF_DATE \n");
		//호텔 및 63 업장 내역조회 2010.07.26#######################################		
		...
		sbSql.append("                   V.SUB_UPJANG, \n");
		sbSql.append("                   (CASE WHEN SUBSTR((SELECT FST_REG_DATE FROM FMS_UPJANG WHERE UPJANG = V.SUB_UPJANG), 1, 4) < SUBSTR('" + str_in_TRX_DATE + "', 1, 4)  THEN '기존' \n");
		sbSql.append("                         WHEN SUBSTR((SELECT FST_REG_DATE FROM FMS_UPJANG WHERE UPJANG = V.SUB_UPJANG), 1, 4) >= SUBSTR('" + str_in_TRX_DATE + "', 1, 4) THEN '신규' \n");
		sbSql.append("                         ELSE '미지정' END) NEW_GUBUN, \n");
		sbSql.append("                   V.TRX_DATE, \n");
		sbSql.append("                   0 QTY, \n");
		...
		sbSql.append("                    AND   (V.TRX_DATE BETWEEN '" + str_in_TRX_DATE + "'||'01' AND '" + str_in_TRX_DATE + "'||'31') \n");		
		...
		sbSql.append("                   NULL SUB_UPJANG, \n");
		sbSql.append("                   (CASE WHEN SUBSTR((SELECT OPEN_DATE FROM SCO_UPJANG_MST WHERE UPJANG = V.UPJANG), 1, 4) < SUBSTR('" + str_in_TRX_DATE + "', 1, 4)  THEN '기존' \n");
		sbSql.append("                         WHEN SUBSTR((SELECT OPEN_DATE FROM SCO_UPJANG_MST WHERE UPJANG = V.UPJANG), 1, 4) >= SUBSTR('" + str_in_TRX_DATE + "', 1, 4) THEN '신규' \n");
		sbSql.append("                         ELSE '미지정' END) NEW_GUBUN, \n");
		sbSql.append("                   V.TRX_DATE, \n");
		sbSql.append("                   0 QTY, \n");
		sbSql.append("                   0 F_AMT, \n");
		...
		//sbSql.append("    ORDER BY V.SAUP_TYPE, V.GUBUN, V.TM_CD, V.UPJANG, V.SUB_UPJANG, V.NEW_GUBUN \n");
