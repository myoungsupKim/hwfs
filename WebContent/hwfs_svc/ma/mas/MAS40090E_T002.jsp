			...
			for ( int iRow = 0 ; iRow < ds_List.getRowCount() ; iRow++ )
			{
				...
				//if (!isData.equals("0")){	
					if(ds_List.getRowType(iRow) == DataSet.ROW_TYPE_UPDATED){
						...
						sbSql.append("MERGE INTO MAS_UPJANG_M_RATE_MGMT A \n");
						sbSql.append("USING (SELECT REPLACE('" + nullToBlank(ds_List.getString(iRow, "INPUT_YM")) + "', '-', '') AS INPUT_YM \n");
						sbSql.append("            , " + nullToBlank(ds_List.getString(iRow, "UPJANG")) + " AS DOC_CD \n");
						sbSql.append("            , '" + nullToBlank(ds_Cond.getString(0, "GUBUN")) + "' AS GUBUN \n");
						sbSql.append("         FROM DUAL) B                   \n");
						sbSql.append("   ON (A.INPUT_YM = B.INPUT_YM AND  \n");
						sbSql.append("       A.DOC_CD   = B.DOC_CD AND    \n");
						sbSql.append("       A.GUBUN    = B.GUBUN)        \n");
						sbSql.append(" WHEN MATCHED THEN                      \n");
						sbSql.append("      UPDATE                            \n");
						sbSql.append("         SET A.M_RATE    = " + nullToBlank(ds_List.getString(iRow, "M_RATE")) + " \n");
						...
						sbSql.append("             , A.M_RATE \n");
						sbSql.append("             , A.CUSER \n");
						sbSql.append("             , A.CDATE \n");
						sbSql.append("             , A.UUSER \n");
						sbSql.append("             , A.UDATE ) \n");
						sbSql.append("      VALUES ( REPLACE('" + nullToBlank(ds_List.getString(iRow, "INPUT_YM")) + "', '-', '') \n");
						sbSql.append("             , " + nullToBlank(ds_List.getString(iRow, "UPJANG")) + " \n");
						sbSql.append("             , '" + nullToBlank(ds_Cond.getString(0,   "GUBUN")) + "' \n");
						sbSql.append("             , " + nullToBlank(ds_List.getString(iRow, "M_RATE")) + " \n");
