...
<%
	...
	try {
		...
		//String todayDate = new java.text.SimpleDateFormat ("yyyyMMdd").format(new java.util.Date());  //오늘날짜
		String todayDate = ""; //오늘날짜
		...
		sbSql.append(" SELECT TO_CHAR(SYSDATE,'YYYYMMDD') AS TODAY FROM DUAL \n");
		
		try{
			pstmt = conn.prepareStatement(sbSql.toString());	
			rs = pstmt.executeQuery(); 		
			rs.next();
			todayDate = rs.getString("TODAY");
		}	
		catch(Exception e){
			throw new Exception("에러발생: 오늘날짜를 잘못가져왔습니다.");
		}				
		finally{
			if(rs!=null){rs.close();}
		}
		...
		{
			for(j=0; j<v_TARGET_SOURCE_NUM; j++)
			{
				v_ADD_DAY = i+j*v_SOURCE_DAYS;

				//날짜가 그달을 넘을 경우 return 한다.
				//날짜가 TARGET_END_DATE 보다 클 경우 continue 한다. 										
				if((Integer.parseInt(v_TARGET_START_DATE) + v_ADD_DAY) > Integer.parseInt(v_TARGET_END_DATE))
				{
					continue;					
				}
				else
				{
					v_TARGET_MENU_DATE =Integer.toString(Integer.parseInt(v_TARGET_START_DATE)+v_ADD_DAY);
				}
				
				if(Integer.parseInt(v_TARGET_MENU_DATE) <= Integer.parseInt(todayDate))
				{
					continue;	
				}				
				//오늘보다 작은날짜는 스킵한다.
				...
				else if(v_SKIP_YN.equals("0"))
				{
					...
					sbSql.append(" AND   MENU_CD LIKE '"+ v_TARGET_MENU_DATE + "-" + v_HALL_CD + v_SOURCE_MEAL + '%' + v_SOURCE_CORNER +"' \n");
				...
				//System.out.println(v_ADD_DAY);

				//날짜는 원본을 타켓날짜로 변경한  v_TARGET_MENU_CD 사용한다.
				v_TARGET_MENU_CD = "REPLACE(B.MENU_CD,'"+ v_SOURCE_START_DATE + "','"+ v_TARGET_MENU_DATE + "')  AS MENU_CD ";
				...
				stmt.execute(sbSql.toString());
				//FileLog.println("d:\\sb_Sql01.txt", sbSql.toString());
				
				//타겟 아이템 NEED 데이트
				v_NEED_DATE = " FSM_ITEM_NEED_DATE_FUN('"+ v_UPJANG +"', '"+ v_HALL_CD +"', A.MEAL, A.CORNER, '"+ v_TARGET_MENU_DATE +"' ) ";
