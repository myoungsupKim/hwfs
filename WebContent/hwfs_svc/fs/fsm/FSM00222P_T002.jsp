...
<%
	...
	try {
		...
		while(rs.next())
		{
			...
			try{
				if(Integer.parseInt(rs2.getString("R_COUNTER"))>0)
				{continue;}		
			}	
			catch(Exception e){
			}				
			...
			{ 				
			   ...
			   if(v_AFTER_RECIPE_GUBUN.trim().equals("표준"))
			   {
				       ...
				       //1) 레시피 마스터
				          sbSql.delete(0, sbSql.length());
				        sbSql.append("UPDATE FSM_REAL_RECIPE_MST \n");
					    sbSql.append("SET \n");
	        		   ...
	        		   //2) 레시피 자재        		
	        		   //타겟 아이템 NEED 데이트 //   substr(menu_cd,0,8)  ==> menu_date   substr(menu_cd,15,3)==> meal  substr(menu_cd,18,3)==> corner
				       v_NEED_DATE = " FSM_ITEM_NEED_DATE_FUN('"+ v_UPJANG +"', '"+ v_HALL_CD +"', substr('"+ rs.getString("MENU_CD") +"',15,3) , substr('"+rs.getString("MENU_CD")+"',18,3), substr('"+ rs.getString("MENU_CD") +"',0,8) ) ";
					   
	        		   sbSql.delete(0, sbSql.length());
	        		sbSql.append("INSERT INTO FSM_REAL_RECIPE_ITEM(UPJANG, MENU_CD,           RECIPE_CD, ITEM_CODE, ITEM_SEQ, NEED_QTY, EDIBLE_QTY,  CUT_SPEC, NEED_DATE,                        CUSER, CDATE, UUSER, UDATE) ");
	        		sbSql.append("SELECT "+ v_UPJANG     +", '"+ rs.getString("MENU_CD") +"', RECIPE_CD, ITEM_CODE, ITEM_SEQ, NEED_QTY, EDIBLE_QTY, CUT_SPEC,  " + v_NEED_DATE + " AS NEED_DATE, '" + g_EmpNo + "', SYSDATE, '" + g_EmpNo + "',SYSDATE ");
	        		...
	        		sbSql.append("WHERE  RECIPE_CD ='"+ v_AFTER_RECIPE_CD +"' ");
	            	   ...
	            	   //3) 레시피 조리법
	        		   sbSql.delete(0, sbSql.length());
	        		sbSql.append("INSERT INTO FSM_REAL_RECIPE_CUISINE(UPJANG, MENU_CD,       RECIPE_CD, CUISINE_SEQ, CUISINE_DESC, PHOTO_PATH, SPHOTO_PATH, CUSER, CDATE, UUSER, UDATE) ");
		...
		//FSM_PRICE_PERIOD_UPDATE_PRO (  V_UPJANG IN NUMBER, V_FROM_DATE IN VARCHAR2, V_TO_DATE IN  VARCHAR2 )
		 sbSql.delete(0, sbSql.length());
		 sbSql.append(" BEGIN \n");
		 sbSql.append(" FSM_PRICE_PERIOD_UPDATE_PRO("+ v_UPJANG +", '"+ v_START_DATE +"', '"+ v_END_DATE +"'); \n");
		 sbSql.append(" END; ");
 		 stmt.execute(sbSql.toString()); 
