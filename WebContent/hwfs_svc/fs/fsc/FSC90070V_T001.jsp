...
<%
    try {
        ...
        sbSql.append("     , (SELECT COUNT(*) FROM FSC_SAFE_RESULT_MST Z \n");
        ...
        sbSql.append("     , (SELECT NM_KOR FROM HLDC_HR_PERSONAL WHERE SABUN = A.RESP_SABUN1) AS RESP_SABUN1_NM \n");
        sbSql.append("     , (SELECT NM_KOR FROM HLDC_HR_PERSONAL WHERE SABUN = A.RESP_SABUN2) AS RESP_SABUN2_NM \n");
        sbSql.append("     , (SELECT NM_KOR FROM HLDC_HR_PERSONAL WHERE SABUN = A.RESP_SABUN3) AS RESP_SABUN3_NM \n");
        ...
        if (strMuCD.length() != 0)
            sbSql.append(
                    "   AND (SELECT DISTINCT MU_CD FROM HLDC_SC_DEPT_V WHERE A.TM_CD = TM_CD AND ROWNUM = 1) = '" + strMuCD + "' \n");
        if (strTmCD.length() != 0)
