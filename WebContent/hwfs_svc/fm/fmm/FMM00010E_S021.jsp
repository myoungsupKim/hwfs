...
<%
	try	{
		...
		String ST_DATE		= in_vl.getString("SDATE");
...
**/		
		...
		sbSql.append("               AND B.ITEM_CODE LIKE 'F%' \n");
		...
		sbSql.append("               AND '"+ST_DATE+"' BETWEEN A.CONTRACT_START AND NVL(A.CONTRACT_END, '99999999') \n");
		...
		sbSql.append("                      AND SDATE <= "+ST_DATE+" \n");		
...
***/
		...
		sbSql.append("				    '"+ST_DATE+"' AS NEED_DATE , \n");		
		...
		sbSql.append("		     SELECT "+UPJANG+" AS UPJANG, "+ST_DATE+" AS NEED_DATE, A.ITEM_CODE, A.SALE_PRICE \n");
		...
		sbSql.append("              WHERE UPJANG = "+UPJANG+" \n");
		sbSql.append("                AND '"+ST_DATE+"' BETWEEN SDATE AND EDATE \n");
...
// 		sbSql.append("													AND APPLY_DATE  BETWEEN '"+ST_DATE+"'  \n");		
...
<%--
	...
	//	conn = getIRISConn();
		...
		sbSql.append("             ,B.FE, B.NA, B.K, B.VITA_A, B.RETIN, B.B_CAROT, B.THIA, B.RIBO, B.NIACIN \n");
		sbSql.append("             , B.VITA_C, B.CHOLE, B.DISUSE_RATE \n");
		sbSql.append("      FROM (SELECT /*+ USE_NL(B) INDEX(B PO_ITEM_MST_PK) */ B.ITEM_CODE, B.ITEM_NAME, B.ITEM_SIZE, B.PO_UOM, B.TAX_CODE,B.KG_CONVERT_RATE \n");
		...
		sbSql.append("			 AND    "+ST_DATE+" BETWEEN A.CONTRACT_START AND NVL(A.CONTRACT_END, '20201231') \n");		
		sbSql.append("			 AND    A.SALE_PRICE > 0 \n");
		sbSql.append("			 AND    A.USE_YN = 'Y' \n");
