...
***/		
		...
		if (sOtcustcd.equals(""))
		{
			...
			sbSql.append( "     , SUM((DECODE(SAL_OPER_PRICE, 0, SALE_PRICE, SAL_OPER_PRICE))) AS  SALE_PRICE1	\n");
		...
		{			
			...
			sbSql.append( "     , DECODE(A.SAL_OPER_PRICE, 0, A.SALE_PRICE, A.SAL_OPER_PRICE) AS  SALE_PRICE1 \n");
			...
			//sbSql.append( " FROM (SELECT A.* , B.ORIGIN_TYPE, B.ITEM_CLASS2, B.ITEM_CLASS3, B.ITEM_CLASS4 FROM FMS_TRANSACTION_V A, FMS_ITEM_V B WHERE A.ITEM_CODE = B.ITEM_CODE) A,  FMS_UPJANG C \n");
			//속도개선, 뷰 해체 20170504
			sbSql.append( "  FROM (SELECT A.*, B.ORIGIN_TYPE, B.ITEM_CLASS2, B.ITEM_CLASS4 FROM   \n");			
			sbSql.append( "               (SELECT a.trans_id, a.trans_date, a.mg_date, a.source_type, a.mvt, a.trans_type,\n");
			...
			*/
			...
			//sbSql.append( "               AND DECODE (SUBSTR (a.trans_date, 1, 6),SUBSTR (a.mg_date, 1, 6), a.trans_date,a.mg_date) BETWEEN  '" + sSdate + "' AND '" + sEdate + "' \n");
			...
			sbSql.append( "               WHERE TRANS_DATE BETWEEN '" + sSdate + "' AND '" + sEdate + "') \n");			
			sbSql.append( "  A, FMS_ITEM_V B WHERE A.ITEM_CODE = B.ITEM_CODE) A,  FMS_UPJANG C \n");
...
***/
			...
			if (sChk_Date.equals("1"))  //입고일 포함하여 조회 시...
			{
				sbSql.append( " GROUP BY A.TRANS_DATE, A.TRANS_UPJANG, C.UPJANGNM_DISP, A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.TRANS_UOM, A.TAX_CODE, A.SALE_PRICE, A.SAL_OPER_PRICE, A.ORIGIN_TYPE, A.ITEM_CLASS2 \n");   //#42709 필드 추가
			}
			else //입고일 제외하여 조회 시...
			{
				sbSql.append( " GROUP BY A.TRANS_UPJANG, C.UPJANGNM_DISP, A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.TRANS_UOM, A.TAX_CODE, A.SALE_PRICE, A.SAL_OPER_PRICE, A.ORIGIN_TYPE, A.ITEM_CLASS2 \n");   //#42709 필드 추가				
			}
		}
		if (sOtcustcd.equals(""))		
			sbSql.append( "  UNION ALL \n");
		...
		{			
			...
			sbSql.append( "     , A.SALE_PRICE AS  SALE_PRICE1 \n");
			...
			sbSql.append("      ,(SELECT C.CLASS_NAME FROM HLDC_PO_ITEM_CLASS C WHERE A.ITEM_CLASS2 = C.CLASS_CODE)\n");
			sbSql.append("         AS CLASS2_NAME \n");
			sbSql.append( "  FROM (SELECT A.*, B.ORIGIN_TYPE, B.ITEM_CLASS2, B.ITEM_CLASS3, B.ITEM_CLASS4  FROM FMP_OTCUST_PR A, FMP_OTCUST_ITEM B WHERE A.ITEM_CODE = B.ITEM_CODE) A, FMS_UPJANG C \n");			
...
***/
			...
			if (sChk_Date.equals("1"))  //입고일 포함하여 조회 시...
			{
				sbSql.append( " GROUP BY A.NEED_DATE, A.UPJANG,C.UPJANGNM_DISP, A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.PO_UOM, A.TAX_CODE, A.SALE_PRICE, A.ORIGIN_TYPE, A.ITEM_CLASS2 \n");   //#42709 필드 추가
			}
			else //입고일 제외하여 조회 시...
			{
				sbSql.append( " GROUP BY A.UPJANG,C.UPJANGNM_DISP, A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.PO_UOM, A.TAX_CODE, A.SALE_PRICE, A.ORIGIN_TYPE, A.ITEM_CLASS2 \n");   //#42709 필드 추가				
			}
		}			
		if (sOtcustcd.equals(""))
		{
			if (sChk_Date.equals("1"))  //입고일 포함하여 조회 시...
			{
				sbSql.append( ") GROUP BY TRANS_DATE, UPJANG, UPJANGNM_DISP, ITEM_CODE, ITEM_NAME, ITEM_SIZE, PO_UOM, TAX_CODE, ORIGIN_TYPE, CLASS2_NAME \n");  //#42709 필드 추가
			}
			else //입고일 제외하여 조회 시...
			{
				sbSql.append( ") GROUP BY UPJANG, UPJANGNM_DISP, ITEM_CODE, ITEM_NAME, ITEM_SIZE, PO_UOM, TAX_CODE, ORIGIN_TYPE, CLASS2_NAME \n");  //#42709 필드 추가				
			}			
		}
		
		if (sChk_Date.equals("1"))  //입고일 포함하여 조회 시...
		{				
			sbSql.append(" ORDER BY UPJANG, TRANS_DATE, ITEM_CODE \n");
		}
		else //입고일 제외하여 조회 시...
		{
			sbSql.append(" ORDER BY UPJANG, ITEM_CODE \n");				
		}	
