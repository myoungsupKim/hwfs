...
<%
	...
	try
	{
		...
		if (otCust.equals("0000")){	
			...
			sbSql.append(" SELECT (SELECT B.UPJANGNM_DISP FROM FMS_UPJANG B WHERE B.UPJANG = TRANS_UPJANG) AS UPJANGNM_DISP \n");
			...
			sbSql.append("      , ROUND(SALE_AMOUNT) AS AMOUNT \n");
			
			//sbSql.append("      , ROUND(DECODE(SUBSTR(TRANS_TYPE,1,1),'I',1,-1) * DECODE(TAX_CODE, '100', 0.1, 0) * ROUND(OP_PRICE * TRANS_QTY)) AS TAX \n"); 
			sbSql.append("      ,DECODE(FLAG,1                                                                                                                 \n");
			sbSql.append("           			,DECODE(SUBSTR(TRANS_TYPE,1,1),'I',1,-1) * ROUND(DECODE(TAX_CODE,'100',0.1,0) * ROUND(TRANS_QTY * SALE_PRICE)) \n");
			...
			sbSql.append("      ,DECODE(FLAG,1                                                                                                                 \n");
			sbSql.append("           			,DECODE(SUBSTR(TRANS_TYPE,1,1),'I',1,-1) * ROUND(DECODE(TAX_CODE,'100',1.1,1) * ROUND(TRANS_QTY * SALE_PRICE)) \n");
			sbSql.append("           			,DECODE(SUBSTR(TRANS_TYPE,1,1),'I',1,-1) * ROUND(TRANS_QTY * SALE_PRICE + SALE_VAT)                            \n");
			...
			sbSql.append("      , (SELECT B.SUBINV_NAME FROM HLDC_PO_SUBINVENTORY B WHERE B.SUBINV_CODE = A.SUBINV_CODE) AS SUBINV_NAME	\n");
			sbSql.append("      , A.TRANS_DATE	\n");                                                                                                                                                                                                                           
			sbSql.append("      , A.ITEM_CODE	\n");                                                                                                                                                                                                                            
			sbSql.append("      , A.ITEM_NAME	\n");
			//sbSql.append("      , A.ORIGIN_TYPE	\n");
			sbSql.append("      , (SELECT B.ORIGIN_TYPE FROM HLDC_PO_ITEM_MST B WHERE A.ITEM_CODE = B.ITEM_CODE) AS ORIGIN_TYPE	\n");
			...
			sbSql.append("      ,(SELECT B.CLASS_NAME FROM HLDC_PO_ITEM_CLASS B, HLDC_PO_ITEM_MST C WHERE B.CLASS_CODE = C.ITEM_CLASS2 AND C.ITEM_CODE = A.ITEM_CODE  )\n");
			...
			*/
			...
			sbSql.append("    AND LENGTH(A.ITEM_CODE) = 7 \n");
			if(!sdate.equals("") && !edate.equals("")){
	        	sbSql.append("   AND A.TRANS_DATE2 BETWEEN '"+sdate.toString()+"' AND '"+edate.toString()+"' \n");
	        }
	        ...
	        if(!subinv.equals("")){
	        	...
	        	sbSql.append("   AND A.SUBINV_CODE = '"+subinv.toString()+"' \n");
	        }
	        if(!tax.equals("")){
