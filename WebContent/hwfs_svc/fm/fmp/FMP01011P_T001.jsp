...
/*-----------------------------------------------------------------------------
■ 시스템명     : 식재영업/수불관리
■ 프로그램ID   : FMP01011P_T001.jsp
■ 프로그램명   : 사업장별 거래실적집계 상세조회(제출단가)
■ 작성일자     : 2008-07-21
■ 작성자       : 박은규
■ 이력관리     : 2008-07-21
-----------------------------------------------------------------------------*/
...
<%
	try	{
		...
		if (sOtCust.equals("0000") || sOtCust.equals(""))
		{
			...
			sbSql.append("SELECT /*+ INDEX(A PO_TRANSACTION_PT_I8) USE_NL(C) INDEX(C FMU_OP_RATE_PK) */ \n");
			if (sType.equals("001"))
			...
			sbSql.append("     , DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE) AS OP_PRICE \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * ROUND(A.TRANS_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE) ) AS OP_AMOUNT \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * ROUND(DECODE(A.TAX_CODE,'100',A.TRANS_QTY,0) * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE) ) AS OP_TAX \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * ROUND(DECODE(A.TAX_CODE,'100',0.1,0) * ROUND(A.TRANS_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE))) AS OP_VAT \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * ROUND(DECODE(A.TAX_CODE,'100',1.1,1) * ROUND(A.TRANS_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE))) AS OP_TOT \n");

			sbSql.append("     , A.SALE_PRICE AS OP_PRICE1 \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * A.SALE_AMOUNT AS OP_AMOUNT1 \n");
			sbSql.append("     , DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * DECODE(A.TAX_CODE,'100',A.SALE_AMOUNT,0) AS OP_TAX1 \n");
			...
			sbSql.append("  FROM HLDC_PO_TRANSACTION A, FMS_UPJANG B, FMU_SB_PRICE C, FMS_SUBINVENTORY D \n");
		...
		if ((! sOtCust.equals("0000")) || sOtCust.equals(""))
		{
			...
			sbSql.append("     , A.PO_QTY AS TRANS_QTY \n");
			
			sbSql.append("     , DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE) AS OP_PRICE \n");
			sbSql.append("     , ROUND(A.PO_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE)) AS OP_AMOUNT \n");
			sbSql.append("     , ROUND(DECODE(A.TAX_CODE,'100',0.1,0) * ROUND(A.PO_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE))) AS OP_VAT \n");
			sbSql.append("     , ROUND(DECODE(A.TAX_CODE,'100',1.1,1) * ROUND(A.PO_QTY * DECODE(NVL(C.SB_PRICE,0),0,A.SALE_PRICE,C.SB_PRICE))) AS OP_TOT \n");
			...
			sbSql.append("  FROM FMP_OTCUST_PR A, FMS_UPJANG B, FMU_SB_PRICE C, FMS_SUBINVENTORY D \n");
