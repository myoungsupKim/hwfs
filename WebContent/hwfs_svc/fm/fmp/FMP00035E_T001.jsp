...
<%
	try	{
		...
		sbSql.append("     , (SELECT * FROM FMP_ORD_CTRL_DAYS A \n");
		sbSql.append("         WHERE CTRL_DATE LIKE '" + nullToBlank(ds_cond.getString(0, "MONTH")) + "'||'%' \n");
		//본사가 없으면 공통설정
		if (sMainUpjang.equals(""))
		...
		if (! sMainUpjang.equals(""))
		{
			sbSql.append("     , (SELECT * FROM FMP_ORD_CTRL_DAYS \n");
			sbSql.append("         WHERE CTRL_DATE LIKE '" + nullToBlank(ds_cond.getString(0, "MONTH")) + "'||'%' \n");
			sbSql.append("           AND UPJANG = -1) C \n");
		}
		sbSql.append(" WHERE A.RDATE = B.CTRL_DATE(+) \n");
		if (! sMainUpjang.equals(""))
			sbSql.append("   AND A.RDATE = C.CTRL_DATE(+) \n");
...
out_vl.add("fa_Sql", sbSql.toString());
		...
		if (! sOp.equals("1"))
		{
			...
			sbSql.append("         FROM FMP_ORD_CTRL_DAYS \n");
			sbSql.append("        WHERE CTRL_DATE LIKE '" + nullToBlank(ds_cond.getString(0, "MONTH")) + "'||'%' \n");
