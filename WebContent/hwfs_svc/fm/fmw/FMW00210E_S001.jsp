...
<%
	try	{
        ...
        sbSql.append("FROM (                                   \n");
		sbSql.append("SELECT                                                                                  \n");
        sbSql.append("       NVL(B.전일매출,0) - NVL((SELECT SUM(RCV_AMT)+ SUM(DISCOUNT_AMT) FROM FMW_RCV WHERE RCV_DATE <= TO_CHAR(TO_DATE('" + strDATE + "', 'YYYYMMDD') -1,'YYYYMMDD') AND UPJANG =  '" + strUpjang + "' AND HALL_CODE = '" + strHALL_CODE + "' ),0) BEFORE_MISU, \n");
        sbSql.append("       (NVL(A.당일매출,0)) - NVL((SELECT SUM(RCV_AMT)+ SUM(DISCOUNT_AMT) FROM FMW_RCV WHERE RCV_DATE = '" + strDATE + "'  AND UPJANG =  '" + strUpjang + "' AND HALL_CODE = '" + strHALL_CODE + "'),0) TODAY_MISU,                                         \n");
        sbSql.append("       NVL(A.당일매출,0) TODAY_MAECHUL,                                                                                                        \n");
        ...
        sbSql.append("                NVL(SUM(SALE_AMOUNT),0) 당일매출      \n");
        ...
        sbSql.append("         (SELECT NVL(SUM(SALE_AMOUNT),0) 전일매출         \n");
        sbSql.append("            FROM FMW_PO_TRANSACTION         \n");
