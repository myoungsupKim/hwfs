	...
	try {
		...
		for (int i = 0; i < iSaveCnt; i++) {
			...
			if (ds_list2.getRowType(i) == DataSet.ROW_TYPE_INSERTED) {
				...
				//같은 센터 내 동일자재가 있는 경우 체크로 변경 2018.01.23 윤지혜대리 요청
		        ...
		        //효율화 자재 센터코드 필드 삭제, 동일자재 체크 방식 변경 20180903 김호석(윤지혜대리 요청)
		        sbSelSql.append("SELECT ITEM_CODE																				\n");
		        sbSelSql.append("           FROM FSI_EFF_MST A 																																\n");       		
	       		sbSelSql.append("           		WHERE 1=1																	\n");
		        sbSelSql.append("           		AND A.ITEM_CODE = '" + ITEM_CODE + "'																										\n");
			    
				//System.out.println("i : " + i);
				//System.out.println("sbSelSql : " + sbSelSql);
