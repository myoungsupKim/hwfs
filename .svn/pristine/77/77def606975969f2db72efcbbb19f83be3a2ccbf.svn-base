...
<%
	...
	try {
		...
		String COPY_YN         = in_vl.getString("COPY_YN");
		...
		DataSet ds_qustList    = in_dl.get("ds_qustList");	    // 평가항목
		...
		String QUST_CD         = "";
		String QUST_CONTENTS   = "";

		System.out.println("=====================================================");
		System.out.println(COPY_YN);
		System.out.println("=====================================================");
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			sbSql.append("DELETE FSC_CONTRT_CLASS                       \n"); 
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			sbSql.delete(0, sbSql.length());
			sbSql.append("DELETE FSC_CONTRT_FORM_DTL                    \n"); 
			sbSql.append(" WHERE TABLE_TYPE = '" + TABLE_TYPE + "'      \n");
			sbSql.append("   AND FORM_CD    = '" + FORM_CD + "'         \n");
			sbSql.append("   AND CLASS_CD   = '" + CLASS_CD + "'        \n");
		...
		// 평가항목 삭제
		RowCnt = ds_qustList.getRemovedRowCount();
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			TABLE_TYPE    = ds_qustList.getRemovedData(i,"TABLE_TYPE").toString(); 
			FORM_CD       = ds_qustList.getRemovedData(i,"FORM_CD").toString();
			CLASS_CD      = ds_qustList.getRemovedData(i,"CLASS_CD").toString();
			QUST_CD       = ds_qustList.getRemovedData(i,"QUST_CD").toString();      
			
			sbSql.delete(0, sbSql.length());
			sbSql.append("DELETE FSC_CONTRT_QUST_ARTICLE                \n"); 
			sbSql.append(" WHERE TABLE_TYPE = '" + TABLE_TYPE + "'      \n");
			sbSql.append("   AND FORM_CD    = '" + FORM_CD + "'         \n");
			sbSql.append("   AND CLASS_CD   = '" + CLASS_CD + "'        \n");
			sbSql.append("   AND QUST_CD    = '" + QUST_CD + "'         \n");
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			if ( RowStatus == DataSet.ROW_TYPE_INSERTED )
			{
				...
				sbSql.append("  FROM FSC_CONTRT_FORM \n");
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("INSERT INTO FSC_CONTRT_FORM ( \n");
			...
			else if ( RowStatus == DataSet.ROW_TYPE_UPDATED ) 
			{
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("UPDATE FSC_CONTRT_FORM                   \n"); 
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			if ( RowStatus == DataSet.ROW_TYPE_INSERTED )
			{
				// 복사모드일 경우 마스터의 점검표 구분을 입력해 준다.
				if (COPY_YN.equals("Y")) {
					TABLE_TYPE    = nullToBlank(ds_tableList.getString(0,"TABLE_TYPE"));	
				} else {
				...
				WGT_RATE      = nullToBlank(ds_hlClassList.getString(i,"WGT_RATE"));
				NOT_FIT_CNT   = nullToBlank(ds_hlClassList.getString(i,"NOT_FIT_CNT"));
				
				sbSql.delete(0, sbSql.length());
				sbSql.append("INSERT INTO FSC_CONTRT_CLASS (\n");
				sbSql.append("       TABLE_TYPE             \n");
				sbSql.append("     , FORM_CD                \n");
				sbSql.append("     , CLASS_CD               \n");
				sbSql.append("     , CLASS_NM               \n");
				sbSql.append("     , CLASS_TYPE             \n");
				sbSql.append("     , WGT_RATE               \n");
			...
			else if ( RowStatus == DataSet.ROW_TYPE_UPDATED ) 
			{
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("UPDATE FSC_CONTRT_CLASS                    \n"); 
				sbSql.append("   SET CLASS_NM    = '" + CLASS_NM + "'    \n"); 
				sbSql.append("     , WGT_RATE    = " + WGT_RATE + "      \n");
				sbSql.append("     , NOT_FIT_CNT = " + NOT_FIT_CNT + "   \n");
				sbSql.append("     , UUSER       = '" + g_EmpNo + "'     \n"); 
				sbSql.append("     , UDATE       = SYSDATE               \n"); 
				sbSql.append(" WHERE TABLE_TYPE = '" + TABLE_TYPE + "'   \n");
				sbSql.append("   AND FORM_CD = '" + FORM_CD + "'         \n");
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("UPDATE FSC_CONTRT_FORM_DTL \n");
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			if ( RowStatus == DataSet.ROW_TYPE_INSERTED )
			{
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("INSERT INTO FSC_CONTRT_FORM_DTL( \n");
				sbSql.append("	     TABLE_TYPE                \n");
				sbSql.append("	   , FORM_CD                   \n");
				sbSql.append("	   , CLASS_CD                  \n");
			...
			else if ( RowStatus == DataSet.ROW_TYPE_UPDATED ) 
			{
				...
				sbSql.append("UPDATE FSC_CONTRT_FORM_DTL \n");
				sbSql.append("   SET CLASS_NM        = '" + CLASS_NM + "'        \n");
				sbSql.append("	   , CHK_IDCTR       =  " + CHK_IDCTR + "        \n");
		...
		// 평가항목 등록, 수정
		RowCnt = ds_qustList.getRowCount();
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			RowStatus = ds_qustList.getRowType(i);
			if ( RowStatus == DataSet.ROW_TYPE_INSERTED )
			{
				// 복사모드일 경우 마스터의 점검표 구분을 입력해 준다.
				if (COPY_YN.equals("Y")) {
					...
					TABLE_TYPE    = nullToBlank(ds_qustList.getString(i,"TABLE_TYPE"));
				}
				
				if (ds_qustList.getString(i,"FORM_CD") != null)
				{
					FORM_CD   = nullToBlank(ds_qustList.getString(i,"FORM_CD"));
				}
				CLASS_CD      = nullToBlank(ds_qustList.getString(i,"CLASS_CD"));      
				QUST_CD       = nullToBlank(ds_qustList.getString(i,"QUST_CD"));
				QUST_CONTENTS = nullToBlank(ds_qustList.getString(i,"QUST_CONTENTS"));
				
				sbSql.delete(0, sbSql.length());
				sbSql.append("INSERT INTO FSC_CONTRT_QUST_ARTICLE (\n");
				sbSql.append("       TABLE_TYPE                    \n");
				sbSql.append("     , FORM_CD                       \n");
				sbSql.append("     , CLASS_CD                      \n");
				sbSql.append("     , QUST_CD                       \n");
				sbSql.append("     , QUST_CONTENTS                 \n");
				sbSql.append("     , CUSER                         \n");
				sbSql.append("     , CDATE                         \n");
				sbSql.append("     , UUSER                         \n");
				sbSql.append("     , UDATE                         \n");
			...
			else if ( RowStatus == DataSet.ROW_TYPE_UPDATED ) 
			{
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("UPDATE FSC_CONTRT_QUST_ARTICLE                \n"); 
				sbSql.append("   SET QUST_CONTENTS = '" + QUST_CONTENTS + "'\n"); 
				sbSql.append("     , UUSER         = '" + g_EmpNo + "'      \n"); 
				sbSql.append("     , UDATE         = SYSDATE                \n"); 
				sbSql.append(" WHERE TABLE_TYPE = '" + TABLE_TYPE + "'      \n");
				sbSql.append("   AND FORM_CD    = '" + FORM_CD + "'         \n");
