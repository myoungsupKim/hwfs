...
function AddDataSet(dm,store){
	...
	_TraceLn("setList.length==" + setList.length);
	var dataSet = null;
	for(var i=0;i<setList.length;i++){
		var setName = setList[i].getAttribute("NAME");
		if (setName.indexOf("DYNAMIC_SOURCE")==-1)
		{
...
function getXmlElements(dm,xmlDoc){
	try{
		...
		//_TraceLn("aaaaa");
	_TraceLn("setNode.length +  = " +setNode.length);
...
function AddRow(dm,ms,dsInfo,node){
	try{
		...
		//_TraceLn("node.length>>>"+node.length);
		for(var i=0;i<node.length;i++){
			var dataNode = node.item(i).childNodes;
			//var data = new Array();
			var detail = new Array();
			//var index = 0;
			//_TraceLn("dataNode.length>>"+dataNode.length);
			for(var j=0;j<dataNode.length;j++){
				//var tagName = dataNode.item(j).tagName;
				var data = new Array();
				var index = 0;
				var tagName;
				try{tagName = dataNode.item(j).tagName;}catch(e){continue;}
				if(tagName!="DATASET"){ //디테일셋이 아니라면
					//data[j] = dataNode.item(j).text;
					data[index++] = i;
					data[index++] = j;
					data[index++] = dataNode.item(j).text;
					ds.AddRow(data);
					//_TraceLn("added row>>>"+dataNode.item(j).text);
				}else{
