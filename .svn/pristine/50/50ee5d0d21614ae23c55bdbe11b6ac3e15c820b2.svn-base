...
import com.hwfs.sm.stm.dao.TaxInvoiceMgntManualDAO;

 /**
 * 세금계산서 발행을 관리하는 TaxInvoiceMgntManual Service Implementation
 * 
 * @ClassName TaxInvoiceMgntManualServiceImpl.java
 * @Description TaxInvoiceMgntManualServiceImpl Class
 ...
 * </pre>
 */
@Service("/sm/stm/taxInvoiceMgntManualService")
public class TaxInvoiceMgntManualServiceImpl extends DefaultServiceImpl implements TaxInvoiceMgntManualService {
	
	/** LogService */
	private final Logger logger = LoggerFactory.getLogger (this.getClass());

	/** taxInvoiceMgntManualDAO Bean 생성 */
	@Resource(name = "/sm/stm/taxInvoiceMgntManualDAO")
	private TaxInvoiceMgntManualDAO taxInvoiceMgntManualDAO;
	...
	public RecordSet selectList(Map<String, Object> mapParam) throws Exception {
		return taxInvoiceMgntManualDAO.selectList(mapParam);
	}
	...
	*/
	...
	public int saveTaxInvoice(DataSetMap taxShetMst, DataSetMap taxShetDtl) throws Exception {
		int insRowCnt = 0;
		int updRowCnt = 0;
		int delRowCnt = 0;
	...
	public RecordSet selectUpjangInfo(Map<String, Object> mapParam) throws Exception {
		...
		if(mapParam.get("schGb").equals("upjang")){
			...
			return taxInvoiceMgntManualDAO.selectNewhghtUpjangInfo(mapParam);
		}
		
		//고객정보 검색
		return taxInvoiceMgntManualDAO.selectCustInfo(mapParam);
	}
	...
	public RecordSet selectTaxShetDetailList(Map<String, Object> mapParam) throws Exception {
		return taxInvoiceMgntManualDAO.selectTaxShetDetailList(mapParam);
	}
	...
	public RecordSet selectInchargeInfo(Map<String, Object> mapParam) throws Exception {
		return taxInvoiceMgntManualDAO.selectInchargeInfo(mapParam);
	}
	...
	*/
	public int deleteTaxInvoice(DataSetMap list) throws Exception {
		...
		for (int i = 0 ; i < list.size() ; i++) {
			...
			delRowCnt = taxInvoiceMgntManualDAO.deleteTaxShetMst(rowData);
			delRowCnt += taxInvoiceMgntManualDAO.deleteTaxShetDtl(rowData);
		}
	...
	public Map<String, Object> taxShetPublishRequest(DataSetMap list) throws Exception{
		...
		for (int i = 0 ; i < list.size() ; i++) {
			...
			//TaxShetMst 검색
			RecordSet rsMst = taxInvoiceMgntManualDAO.selectList(rowData);
			//TaxShetDtl 검색
			RecordSet rsDtl = taxInvoiceMgntManualDAO.selectTaxShetDetailList(rowData);
...
//			appendDto.setXslNumFilenm("A");
		...
		if (res.isSuccess()) {
			...
			for (int i = 0 ; i < list.size() ; i++) {
				...
				taxInvoiceMgntManualDAO.updateTaxProgressStatus(rowData);
			}
		}
		else {
...
//			sendDto.setSendDetailInfo("");										//송신상세정보
...
//			sendDto.setModifyName("");											//수정성명
	...
	*/
	...
	public Map<String, String> saveConcellation(DataSetMap taxShetMst, DataSetMap taxShetDtl) throws Exception {
		...
		//세금계산서 일련번호 생성
		String taxShetNum = (String)taxInvoiceMgntManualDAO.selectTaxShetNumConcellation(taxShetMstMap);
		taxShetMstMap.put("newtaxShetNum", taxShetNum);
		returnValue.put("concellationTaxNum", taxShetNum);
		
		//세금계산서 마스터 등록
		insRowCnt = taxInvoiceMgntManualDAO.insertConcellation(taxShetMstMap);
		insRowCnt += taxInvoiceMgntManualDAO.insertConcellationDetail(taxShetMstMap);
		...
		if(!StringUtils.isEmpty(modifyCd) && (modifyCd.equals("01") || modifyCd.equals("02"))){
			//세금계산서 일련번호 생성
			taxShetNum = (String)taxInvoiceMgntManualDAO.selectTaxShetNumConcellation(taxShetMstMap);
			taxShetMstMap.put("newtaxShetNum", taxShetNum);
			...
			if(modifyCd.equals("01")){
				for(int idx=0; idx<taxShetDtl.size(); idx++){
					...
					insRowCnt += taxInvoiceMgntManualDAO.insertTaxShetCorrectionDtl(taxShetDtlMap);
				}
				...
				for(int idx=0; idx<taxShetDtl.size(); idx++){
					...
					insRowCnt += taxInvoiceMgntManualDAO.insertTaxShetApplyAmtDtl(taxShetDtlMap);
				}
			}
			...
			//세금계산서 마스터 등록
			insRowCnt += taxInvoiceMgntManualDAO.insertTaxShetCorrectionMst(taxShetMstMap);
