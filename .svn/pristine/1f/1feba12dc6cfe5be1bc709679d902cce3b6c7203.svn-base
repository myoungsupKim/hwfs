...
<%
	try	{
		...
		*/
		...
		sbSql.append("     , SUM(DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * ROUND(ROUND(A.TRANS_QTY * DECODE(A.SAL_OPER_PRICE, 0, A.SALE_PRICE, A.SAL_OPER_PRICE)) * DECODE(A.TAX_CODE,'100',1.1,1))) AS OP_AMOUNT1 \n");
...
//**************  FM_NOSALE_RATE 추가에 따른 변경  2009/07/22 백업 및 수정  *******************
		sbSql.append("  FROM FMS_TRANSACTION_V A, FMS_SALE_RATE B  \n");
		/*
		sbSql.append("     , FMU_OP_RATE C \n");
		*/
		sbSql.append("     , FMS_UPJANG D, FMS_UPJANG E \n");
		...
		sbSql.append("   AND B.CASH_CODE(+) = '003' \n");  // 장려금		
