...
$(document).ready(function (){
	...
	$("#btn6").bind("click", function() {
		$.ajax({
			  type : "POST"
			, url : "http://localhost:8080/mb/cmn/mbCommon.jDo"
			, dataType : "json"
			, contentType : "application/json"
			//5.식단정보
			, data : "{'txHeader':{'txId':'IF_MOBILE_LOGIN', 'encoding':'utf-8', 'encYn':'false', 'txDiv':'SELECTUSER'}, 'txData':{'data':{'id':'210302', 'connIp':'128.0.0.1', 'pageNo': 1}}, 'txResult':{}}"
			 , success : function (data) {
				 alert(data.txResult.resultCode + "\n" + data.txResult.resultMessage + "\n" +data.txData);
		    }
		});
...
<input type="button" id="btn6" name="btn6" value="6.로그인정보" />
