...
<%
	...
	//	String sCode = in_ds.getString(0, "CD");
			...
			String PRCS_STATUS 		= nullToBlank(in_vl.getString("PRCS_STATUS"));
		...
		if(!PRCS_STATUS.equals("") )
		{
			sbSql.append("		   AND A.PRCS_STATUS '"+PRCS_STATUS+"' --처리상황\n");
		}
		...
		if(!TO.equals("") )
		{
			sbSql.append("		   AND A.NEED_DATE <= '"+TO+"'    --입고일\n");
		}
		...
		DataSet ds_tab2 = this.makeDataSet(rs,"ds_tab2");
		out_dl.add(ds_tab2);
