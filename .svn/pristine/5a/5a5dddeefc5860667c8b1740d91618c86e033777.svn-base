...
<%
	try	{
        ...
        sbSql.append("             AND C.HALL_CODE = T.HALL_CODE)),0) AS 전일미수                                                          \n");
        ...
        sbSql.append("      ,SUM(DECODE(A.TRANS_TYPE,'O001', DECODE(C.TAX_CODE,'100',1.1,1) * NVL(A.TRANS_QTY * A.SALE_PRICE,0),0)) AS 매출 \n");
        sbSql.append("      ,SUM(DECODE(A.TRANS_TYPE,'O002', DECODE(C.TAX_CODE,'100',1.1,1) * NVL(A.TRANS_QTY * A.SALE_PRICE,0),0)) AS 반품 \n");
        sbSql.append("      ,SUM(DECODE(A.TRANS_TYPE,'O001', NVL(A.TRANS_QTY * A.SALE_PRICE,0),0)) -                                    \n");
        sbSql.append("       SUM(DECODE(A.TRANS_TYPE,'O002', NVL(A.TRANS_QTY * A.SALE_PRICE,0),0)) AS 순매출                            \n");
