...
import com.hwfs.rc.rrm.dao.StandardRecipeInquirybyRecipeDAO;

 /**
 * 표준 레시피를 조회하는 StandardRecipeInquirybyRecipe Service Implementation
 * 
 * @ClassName StandardRecipeInquirybyRecipeServiceImpl.java
 * @Description StandardRecipeInquirybyRecipeServiceImpl Class
 ...
 * </pre>
 */
@Service("/rc/rrm/standardRecipeInquirybyRecipeService")
public class StandardRecipeInquirybyRecipeServiceImpl extends DefaultServiceImpl implements StandardRecipeInquirybyRecipeService {
	
	/** LogService */
//	private final Logger logger = LoggerFactory.getLogger (this.getClass());

	/** StandardRecipeInquirybyRecipeDAO Bean 생성 */
	@Resource(name = "/rc/rrm/standardRecipeInquirybyRecipeDAO")
	private StandardRecipeInquirybyRecipeDAO standardRecipeInquirybyRecipeDAO;
	...
	public RecordSet selectList(Map<String, Object> mapParam) throws Exception {
		...
		RecordSet rs = standardRecipeInquirybyRecipeDAO.selectRecipe(mapParam);
		
		int level = Integer.parseInt(mapParam.get("treeLevel").toString());
		
		//레시피 level1 검색
		mapParam.put("treeLevel", "1");
		RecordSet rsDtl = standardRecipeInquirybyRecipeDAO.selectList(mapParam);
		...
		if(rs != null && rs.size() > 0){
			...
			for(int i=0; i<rs.size(); i++){
				...
				if(record.get("semiGoodsYn").equals("Y")){
					...
					rsTemp = standardRecipeInquirybyRecipeDAO.selectList(mapParam);
