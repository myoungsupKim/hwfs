...
function Datepicker() {
	...
	this.dpDiv = $('<div id="' + this._mainDivId + '" style="display: none; z-index:1;"></div>');
}
...
$.extend(Datepicker.prototype, {
		...
		if (showOn == 'button' || showOn == 'both') { // pop-up date picker when button clicked
			...
			var trigger = $(this._get(inst, 'buttonImageOnly') ? 
				$('<img/>').addClass(this._triggerClass).
					attr({ src: buttonImage, alt: buttonText, title: buttonText, style:"cursor:hand" }) :
	...
	_generateDatepicker: function(inst) {
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dow = 0; dow < 7; dow++) { // days of the week
					...
					html += '<td' + ((dow + firstDay + 6) % 7 == 6 ? ' class="ui-datepicker-week-sunday"' : ((dow + firstDay + 6) % 7 == 5 ? ' class="ui-datepicker-week-saturday"' : '')) + '>' +
						(!changeFirstDay ? '<span' :
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						html += '<td class="ui-datepicker-days-cell' +
							((dow + firstDay + 6) % 7 == 6 ? ' ui-datepicker-week-sunday' : ((dow + firstDay + 6) % 7 == 5 ? ' ui-datepicker-week-saturday ' : '')) + // highlight weekends
		...
		html += (showStatus ? '<div style="clear: both;"></div><div id="ui-datepicker-status-' + inst.id + 
			...
			'<iframe src="javascript:false;"  class="ui-datepicker-cover" style="background-color:#000000;padding-left:0px ! important;min-width:0px ! important;min-height:0px ! important;"></iframe>';
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		//yearRange에 +- 범위 뿐만 아니라 시작년도, 종료년도를 병행 하여 사용 할 수 있게 사용 하기 위해 수정.(20110106)
		if (secondary || !this._get(inst, 'changeYear'))
			...
			if (years.length != 2) {
				...
				if (years[1].charAt(0) == '+' || years[1].charAt(0) == '-') {
			...
			if (year > drawYear) year = drawYear;
			if (endYear < drawYear) endYear = drawYear;
...
$.datepicker.setDefaults({
	...
	currentText: '오늘',
	clearText: '지우기',
	prevText: '&#x3c;전월', // Display text for previous month link
	nextText: '다음월&#x3e;', // Display text for next month link
	yearRange: '-10:+0',
	buttonImageOnly: true,
	buttonText: "달력",
	buttonImage: "/img/svc/btn_calendar.gif"
});