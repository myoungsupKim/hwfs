...
<%
	...
	try {
		...
		String str_in_CC_CD = nullToBlank(ds_cond.getString(0,
				"CC_CD"));		
		String str_in_chk_0 = nullToBlank(ds_cond.getString(0, "chk_0"));			// 판매수수료 제외
		String str_in_chk_1 = nullToBlank(ds_cond.getString(0, "chk_1"));			// 본사집계로 조회
		...
		sbSql.append("              SUM(PRE_SALE_VAT) -  SUM(RS.PRE_CK_SALE_VAT + RS.PRE_JI_SALE_VAT) AS PRE_SALE_ALL    --매출+부가세                                                                  \n");
		sbSql.append("            , SUM(PRE_SALE_NOTAX) - SUM(RS.PRE_CK_SALE_NOTAX + RS.PRE_JI_SALE_NOTAX) +                                                                                          \n");
		sbSql.append("              SUM(PRE_SALE_TAX) - SUM(RS.PRE_CK_SALE_TAX + RS.PRE_JI_SALE_TAX) AS PRE_SALE_TOT     --매출액                                                                       \n");
		sbSql.append("            , SUM(PRE_SALE_NOTAX) - SUM(RS.PRE_CK_SALE_NOTAX + RS.PRE_JI_SALE_NOTAX) +                                                                                          \n");
		sbSql.append("              SUM(PRE_SALE_TAX) - SUM(RS.PRE_CK_SALE_TAX + RS.PRE_JI_SALE_TAX)  -                                                                                                 \n");
		sbSql.append("              SUM(PRE_TRANS_SUM)  AS PRE_MARGIN                                                                         --이익액                                                                       \n");
		...
		sbSql.append("              SUM(SALE_VAT) -  SUM(RS.CK_SALE_VAT + RS.JI_SALE_VAT) AS SALE_ALL    --매출+부가세                                                                                      \n");
		sbSql.append("            , SUM(SALE_NOTAX) - SUM(RS.CK_SALE_NOTAX + RS.JI_SALE_NOTAX) +                                                                                                          \n");
		sbSql.append("              SUM(SALE_TAX) - SUM(RS.CK_SALE_TAX + RS.JI_SALE_TAX) AS SALE_TOT     --매출액                                                                                           \n");
		sbSql.append("            , SUM(SALE_NOTAX) - SUM(RS.CK_SALE_NOTAX + RS.JI_SALE_NOTAX) +                                                                                                          \n");
		sbSql.append("              SUM(SALE_TAX) - SUM(RS.CK_SALE_TAX + RS.JI_SALE_TAX)  -                                                                                                                 \n");
		sbSql.append("              SUM(TRANS_SUM)  AS MARGIN                                                             --이익액                                                                                           \n");
...
sbSql.append("                        , CASE WHEN X.TRANS_DATE BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE("+ str_in_TRANS_DATEFR +",'YYYYMMDD'),-1),'YYYYMMDD') AND TO_CHAR(ADD_MONTHS(TO_DATE("+ str_in_TRANS_DATETO +",'YYYYMMDD'),-1),'YYYYMMDD') THEN  \n");
		...
		sbSql.append("                                               , FMS_ITEM_SALE_RATE_CODE_FUN(A.TAX_CODE, A.TRANS_UPJANG, A.SUBINV_CODE, '001', A.ITEM_CODE, A.TRANS_DATE) AS C_SALE_RATE        /* 자재별 CK 비율 */   \n");
														}else{
		sbSql.append("                                              , 0 AS C1_SALE_RATE                                /* 분류 기준 CK 물류비율 */                                                                           \n");
		sbSql.append("                                              , 0 AS C_SALE_RATE                                    /* 자재 기준 CK 물류비율 */                                                                        \n");
														}
		sbSql.append("                                             /*검색 조건 : 지급수수료제외*/                                                                                                                            \n");
