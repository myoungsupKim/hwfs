...
<%
    ...
    try {
        ...
        DataSet ds_item_condi = in_dl.get("ds_item_condi");

        String EXPT_UPJANG_YN = nullToBlank(ds_item_condi.getString(0, "EXPT_UPJANG_YN"));   // 추가구매승인제외업장YN
        String APPROV_YN      = nullToBlank(ds_item_condi.getString(0, "APPROV_YN"));   // 추가구매승인YN
        String ITEM_CODE      = nullToBlank(ds_item_condi.getString(0, "ITEM_CODE"));   // 조회구분
        String ITEM_CLASS1    = nullToBlank(ds_item_condi.getString(0, "ITEM_CLASS1")); // 조회구분
        String ITEM_CLASS2    = nullToBlank(ds_item_condi.getString(0, "ITEM_CLASS2")); // 조회구분
        String ITEM_CLASS3    = nullToBlank(ds_item_condi.getString(0, "ITEM_CLASS3")); // 조회구분
        String ITEM_CLASS4    = nullToBlank(ds_item_condi.getString(0, "ITEM_CLASS4")); // 조회구분
        String BARCODE        = nullToBlank(ds_item_condi.getString(0, "BARCODE"));
        String PR_DATE        = nullToBlank(ds_item_condi.getString(0, "PR_DATE"));
        String NEED_DATE      = nullToBlank(ds_item_condi.getString(0, "NEED_DATE"));
        String CUSTCD         = nullToBlank(ds_item_condi.getString(0, "CUSTCD"));
        String SUBINV_CODE    = nullToBlank(ds_item_condi.getString(0, "SUBINV_CODE"));
        ...
        if(!nullToBlank(ITEM_CODE).equals(""))
        {
// [속도개선 Project 시작] 2017. 8. 28. 최범주 쿼리 변수 바인딩기능 추가        	
            String[] arrSameWord_tmp = nullToBlank(ITEM_CODE).split(" ");

            sbSql_tmp.setLength(0);
            sbSql_tmp.append("/* fs/fsp/FSP00035E_T002.jsp */                                                 \n");
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주
        ...
        sbSelSql.append("     , 'N' CHEAP_YN                                                 /* 저럼한자재유무 */                                                                                               \n");
        ...
        //속도 문제로 뷰사용하는G 대신 동일한 함수호출로 변경 2017.05.02 김호석(선조치) 예전 소스에 사용했던걸로 추정
		sbSelSql.append("     , CASE WHEN EPROCUSR.FN_ITEM_PROCURE_REQ_WKD('100', 'HFC', F.OPER_ORG_SN_PURC, A.ITEM_CODE, A.VD_SN, '01', '" + NEED_DATE +    "') = 'XR' THEN 'Y' ELSE 'N' END  AS PR_YN       \n");      
		sbSelSql.append("     , '' AS SCH_INFO                                                                                                                                                                      \n");
        ...
        sbSelSql.append("     , DECODE(C.D_DAYS,NULL,   0, DECODE(SIGN(TO_DATE('" + NEED_DATE + "', 'YYYYMMDD') - TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD') - NVL(C.D_DAYS,0) ) , 0,                    \n");
        sbSelSql.append("                                  DECODE(SIGN(DECODE(C.D_TIMES, '', 24, 0, 24, C.D_TIMES) - TO_NUMBER(TO_CHAR(SYSDATE, 'HH24'))), 1, 0, 1), 0) + NVL(C.D_DAYS, 0) )                    \n");
        ...
        sbSelSql.append("     , SCC_SOPR_AGG_FUN('"	+ SUBINV_CODE +	"',	A.ITEM_CODE, 1)	AS AGG_QTY	/* 평균발주수량 */																							\n");
        sbSelSql.append("     , A.UPJANG_GRP                                                 /* 업장군 */                                                                                                        \n");
        sbSelSql.append("     , A.FAVORITE_CNT                                                /* 즐겨찾기 등록되있는 갯수 */                                                                                                      \n");
        ...
        sbSelSql.append("              , CASE WHEN (SELECT COUNT(1) FROM FSP_FAVORITE X WHERE X.UPJANG = A.UPJANG AND X.ITEM_CODE = C.ITEM_CODE AND X.USE_YN = 'Y') > 0 THEN '등록완료' ELSE '추가하기' END AS FAVORITE_CNT     /* 즐겨찾기 등록되있는 갯수 */    \n");
        ...
        if(!CUSTCD.equals(""))
            sbSelSql.append("            AND A.CUSTCD      = " + CUSTCD  + "                                                                                                                                    \n");
        ...
        sbSelSql.append("            AND C.ITEM_CLASS1  = '" + ITEM_CLASS1 + "'                                                                                                                                 \n");
        if(!BARCODE.equals(""))
            sbSelSql.append("            AND C.BARCODE  = '" + BARCODE  + "'                                                                                                                                    \n");
        if (! ITEM_CODE.equals(""))
        {
            sbSelSql.append("            AND (      \n");
            for(int i=0; i<strKeyword_ItemName.length; i++) {
                sbSelSql.append("                 (                                                                                                                                                             \n");
                sbSelSql.append("                   C.ITEM_CODE = UPPER('" + strKeyword_ItemName[i] + "')                                                                                                       \n");
                sbSelSql.append("                   OR                                                                                                                                                          \n");
                sbSelSql.append("                   C.ITEM_NAME LIKE '%'||'" + strKeyword_ItemName[i] + "'||'%'                                                                                                 \n");
                sbSelSql.append("                 )                                                                                                                                                             \n");
                if(i < strKeyword_ItemName.length-1) {
                    sbSelSql.append("             OR \n");
                }
            }
            /*2016.05.25 최학진 동의어가 있을 경우 기본적으로 검색 데이터도 조회되게 수정 FS메뉴지원팀 윤지혜 대리 요청*/
            sbSelSql.append("                   OR                                                                                                                                                          \n");
            sbSelSql.append("                   C.ITEM_NAME LIKE '%'||'" + ITEM_CODE + "'||'%'                                                                                                 \n");
            /*2016.05.25 최학진 동의어가 있을 경우 기본적으로 검색 데이터도 조회되게 수정 FS메뉴지원팀 윤지혜 대리 요청 끝*/
            sbSelSql.append("                ) \n");
        }
        else
        {
            if(!ITEM_CLASS2.equals(""))
                sbSelSql.append("            AND C.ITEM_CLASS2 = '" + ITEM_CLASS2 + "'                                                                                                                          \n");
            if(!ITEM_CLASS3.equals(""))
                sbSelSql.append("            AND C.ITEM_CLASS3 = '" + ITEM_CLASS3 + "'                                                                                                                          \n");
