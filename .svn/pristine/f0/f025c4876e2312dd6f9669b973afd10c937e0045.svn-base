...
/*-----------------------------------------------------------------------------
■ 시스템명     : 사업계획관리
■ 프로그램ID   : MAP30210E_T002.jsp
■ 프로그램명   : 마케터별 사업계획관리 저장
■ 작성일자     : 2015-08-6
■ 작성자       : 김태윤
■ 이력관리     : 
-----------------------------------------------------------------------------*/
...
<%
	try	{
		...
		if("0".equals(isData)){	
			...
			sbSql.append("		 (UPJANG, YY, BIZ_CLASS, CONFIRM_YN, IMPROV_YN, ORGIN_YN						\n");
			sbSql.append("		 ,VER_CD, TM_CD, SABUN, CDATE, CUSER)											\n");
			sbSql.append("VALUES ('" + nullToBlank(ds_Cond.getString(0, "CC_CD")) + "'			 				\n");
			...
			sbSql.append("      , '" + nullToBlank(ds_Cond.getString(0, "SABUN")) + "', SYSDATE, '" + g_EmpNo + "' ) \n");
		}
			pstmt = conn.prepareStatement(sbSql.toString());
			rs = pstmt.executeQuery();
			...
			for ( int iRow = 0 ; iRow < ds_List.getRowCount() ; iRow++ )
			{
				...
				sbSql.delete(0, sbSql.length());
				sbSql.append("MERGE INTO MAP_BIZ_PLAN2 A 															\n");
				...
				sbSql.append("       A.BIZ_CLASS    = B.BIZ_CLASS  )												\n");
