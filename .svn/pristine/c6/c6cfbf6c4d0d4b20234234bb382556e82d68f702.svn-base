 ...
 * </pre>
 */
public class BankAcctNoRequestIfSchdJob  extends DefaultQuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        UpjangMgmtNewService upjangMgmtNew;

        try {
            scheduleService = (ScheduleService)context.getJobDetail().getJobDataMap().get("scheduleService");
            upjangMgmtNew = (UpjangMgmtNewService)context.getJobDetail().getJobDataMap().get("upjangMgmtNewService");
            //스케쥴러가 실행가능한지 검사
            if(scheduleService.isScheduleExcute(execServer)){
                int cnt = upjangMgmtNew.procSapRequest();
