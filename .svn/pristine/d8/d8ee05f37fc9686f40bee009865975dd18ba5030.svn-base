...
<%
	try	{
		...
		sbSql.append("       A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.TRANS_UOM \n");
		sbSql.append("     , SUM(DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * A.TRANS_QTY) AS TRANS_QTY \n");
		sbSql.append("     , SUM(DECODE(SUBSTR(A.TRANS_TYPE,1,1),'I',1,-1) * A.TRANS_MAMOUNT) AS TRANS_MAMOUNT \n");
		...
		sbSql.append(" GROUP BY A.ITEM_CODE, A.ITEM_NAME, A.ITEM_SIZE, A.TRANS_UOM \n");
		if ((!FAMT.equals("")) && (!TAMT.equals("")))
		{
			...
			sbSql.append(" HAVING SUM(A.TRANS_MAMOUNT) <= " + TAMT + " \n");
