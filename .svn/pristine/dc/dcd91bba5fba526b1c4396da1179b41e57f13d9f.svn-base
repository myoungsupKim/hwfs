...
<%
    try {
        ...
        sbSql.append("     , (SELECT MAN_SCORE FROM FSC_FORM_MST WHERE TABLE_TYPE = '" + strTableType + "' AND FORM_CD = '" + strFormCD + "') AS MAN_SCORE\n");
        ...
        sbSql.append("     , (SELECT MNGM_CNT FROM FSC_CLASS_MST WHERE TABLE_TYPE = A.TABLE_TYPE AND FORM_CD = A.FORM_CD AND CLASS_CD = A.MIDDLE_CLASS_CD) MNGM_CNT \n");
