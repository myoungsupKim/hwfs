...
import com.hwfs.rc.sle.service.UserMgmtService;

 /**
 * UserMgmtController에 대한 설명 작성
 ...
 */
@Controller
@RequestMapping("/rc/sle/userMgmt/")
public class UserMgmtController extends DefaultController {
	@Resource(name="/rc/sle/userMgmtService")
	private UserMgmtService userMgmt;
	...
	public ModelAndView selectList(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		RecordSet rs = userMgmt.selectList(parameter);
		
		//개인정보보호 접근로그 조회 조건과 건수
		//accessLogService.readLog(parameter, rs.size());
				
		//XPlatform으로 넘겨줄 Dataset을 설정한다.
		super.addOutDataSet(mav, "ds_userMst", rs);
	...
	public ModelAndView saveData(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		int procCnt =  userMgmt.saveData(parameter.get("ds_userMst"));
	...
	@RequestMapping("updateDataPwd.xdo")
	public ModelAndView updateDataPwd(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		//ModeAndView를 생성한다.
		ModelAndView mav = super.createModelAndView();
		...
		int procCnt =  userMgmt.updateDataPwd(parameter.get("ds_userMst"));
