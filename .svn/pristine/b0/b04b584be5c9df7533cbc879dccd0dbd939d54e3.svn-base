...
<%
	...
	try {
		...
		if (DelRowCnt > 0)	// 1. 삭제
		{
		  	String strUpjang		= ds_Master.getRemovedData(0, "UPJANG").toString();
		  	String strVisitDt 		= ds_Master.getRemovedData(0, "VISIT_DT").toString();
		  	Integer intSeqNo 		= Integer.parseInt(ds_Master.getRemovedData(0, "SEQNO").toString());
			...
			sbSql.append("SELECT COUNT(*) AS CNT \n");
			sbSql.append("  FROM FSC_TEST_EXTERNAL_DTL \n");
			sbSql.append("WHERE UPJANG = '" + strUpjang + "' \n");
			sbSql.append("  AND VISIT_DT = '" + strVisitDt + "' \n");
			sbSql.append("  AND SEQNO = " + intSeqNo + " \n");
			...
			sbSql.delete(0, sbSql.length());
			sbSql.append("SELECT COUNT(*) AS CNT \n");
			sbSql.append("  FROM FSC_TEST_EXTERNAL_DTL_PICK \n");
			sbSql.append("WHERE UPJANG = '" + strUpjang + "' \n");
			sbSql.append("  AND VISIT_DT = '" + strVisitDt + "' \n");
			sbSql.append("  AND SEQNO = " + intSeqNo + " \n");
			...
			// 삭제
			sbSql.delete(0, sbSql.length());
			sbSql.append("DELETE FROM FSC_TEST_EXTERNAL_MST \n");
			sbSql.append("WHERE UPJANG = '" + strUpjang + "' \n");
			sbSql.append("  AND VISIT_DT = '" + strVisitDt + "' \n");
			sbSql.append("  AND SEQNO = " + intSeqNo + " \n");
