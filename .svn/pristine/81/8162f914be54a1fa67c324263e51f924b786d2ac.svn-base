<%
	/*-----------------------------------------------------------------------------
	 ■ 시스템명	: 구매관리/구매신청/구매신청서작성/식자재구매신청(확정 시 취소 체크)
	 ■ 프로그램ID	: FSP00032E_T014.jsp
	 ■ 프로그램명	: 구매신청(신청된건에 대한 자재수정일경우)
	 ■ 작성일자	: 2018-03-13
	 ■ 작성자	: 김호석
	 ■ 이력관리	: 2018-03-13	 
	 -----------------------------------------------------------------------------*/
...
<%
	...
	try {
		...
		DataSet ds_condi = in_dl.get("ds_condi3"); // 조회조건및 필수 파라미터	데이터셋
		DataSet ds_pr_item = in_dl.get("ds_pr_item3"); // 구매신청자제	데이터셋
		DataSet ds_del_item = in_dl.get("ds_del_item3"); // 식단에서	구매신청에서 제외된	자재
		...
		//진행상태 체크(저장전 상태가 바뀐경우) 2018.03.13 김호석
			...
			for (int i = 0; i < iSaveCnt; i++) {				
				...
				if (ds_pr_item.getRowType(i) == DataSet.ROW_TYPE_UPDATED || ds_pr_item.getRowType(i) == DataSet.ROW_TYPE_NORMAL) {
					...
					if ((ds_pr_item.getString(i, "PR_ID").equals(s_prid)) && (!ds_pr_item.getString(i, "LINE_STATUS").equals(s_status)))  {
						if (s_status.equals("003") || s_status.equals("999") || s_status.equals("PD")) {					
							s_cnt += 1;	
							itemlist = itemlist + " \n - " + s_itemname;
						}
					}
