...
<%
	...
	try {
		...
		if (DelRowCnt > 0)	// 1. 삭제
		{
			strTableType = ds_Master.getRemovedData(0, "TABLE_TYPE").toString();
			...
			sbSql.append("DELETE FROM FSC_FORM_MST \n");
			sbSql.append("WHERE TABLE_TYPE = '" + strTableType + "' \n");
			sbSql.append("  AND FORM_CD = '" + strFormCD + "' \n");
			...
			// 코드삭제
			sbSql.delete(0, sbSql.length());
			sbSql.append("DELETE FROM FSC_CLASS_MST \n");
			sbSql.append("WHERE TABLE_TYPE = '" + strTableType + "' \n");
			sbSql.append("  AND FORM_CD = '" + strFormCD + "' \n");
			...
			// 양식디테일삭제
			sbSql.delete(0, sbSql.length());
			sbSql.append("DELETE FROM FSC_FORM_DTL \n");
			sbSql.append("WHERE TABLE_TYPE = '" + strTableType + "' \n");
			sbSql.append("  AND FORM_CD = '" + strFormCD + "' \n");
