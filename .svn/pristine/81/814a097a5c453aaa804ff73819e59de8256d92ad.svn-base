...
import com.hwfs.sc.app.dao.AdminDAO;

 /**
 * 식재앱 관리자 인터페이스용  ServiceImpl에 대한 설명 작성
 * @ClassName AdminServiceImpl.java
 * @Description AdminServiceImpl Class 
	...
	*/
	...
	public JSONObject callService(JSONObject txHeader, JSONObject txData,JSONObject txResult) throws Exception {
		...
		if(txDiv.equals("SELECT_TEST")){
			...
			rs = adminDAO.selectTest(parameter);
		...
		else if(txDiv.equals("SelectUpjangInfo"))
		{
			...
			rs = adminDAO.selectUpjang(parameter);
		...
		//품목조회
		else if(txDiv.equals("SelectItemClass"))
		{
			...
			rs = adminDAO.selectItemClass(parameter);
			...
			if(rs == null) {
				...
				procResult.put("ErrorCode", -1); //오류
				procResult.put("ErrorMsg", "품목 조회 중 오류가 발생했습니다.");
			} else {
				rs = removeNull(rs);
				...
				if(rs.size() == 0) //사용자정보없는경우
				{
					...
					procResult.put("resultCount", 0);
					procResult.put("ErrorCode", -1); //오류
					procResult.put("ErrorMsg", "품목정보가 존재하지 않습니다.");
				}
				else
				{
					JSONArray rsArray = new JSONArray(rs);
		...
		//센터조회
		else if(txDiv.equals("SelectCenterInfo"))
		{
			...
			rs = adminDAO.selectCenterInfo(parameter);
			...
			if(rs == null) {
				...
				procResult.put("ErrorCode", -1); //오류
				procResult.put("ErrorMsg", "센터정보 조회 중 오류가 발생했습니다.");
			} else {
				rs = removeNull(rs);
				...
				if(rs.size() == 0) //사용자정보없는경우
				{
					...
					procResult.put("resultCount", 0);
					procResult.put("ErrorCode", -1); //오류
					procResult.put("ErrorMsg", "센터정보가 존재하지 않습니다.");
				}
				else
				{
					JSONArray rsArray = new JSONArray(rs);
		...
		insLogCnt += adminDAO.logInsert(parameter);
		//로그저장end
