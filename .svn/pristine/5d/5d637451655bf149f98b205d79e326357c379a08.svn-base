...
import com.hwfs.fm.fms.service.ThemeItemProfitService;
import com.hwfs.sc.cmn.dto.LoginDTO;


 /**
 * 테마자재율 등록하는 ThemeItemProfit Controller
 * 
 * @ClassName ThemeItemProfitController.java
 * @Description ThemeItemProfitController Class 
 ...
 */
@Controller
@RequestMapping("/fm/fms/ThemeItemProfit/")
public class ThemeItemProfitController extends DefaultController {
	
	/** ThemeItemProfitService Bean 생성 */
	@Resource(name = "/fm/fms/ThemeItemProfitService")
	private ThemeItemProfitService themeItemProfitService;
	//private ThemeItemProfitService ThemeItemProfitService;
	...
	public ModelAndView selectStd(
		...
		//테마자재율 등록 을(를) 조회한다.
		RecordSet rs = themeItemProfitService.selectStd(parameter);
	...
	public ModelAndView selectClassType(
		...
		//자재분류정보를 조회한다.
		RecordSet rs = themeItemProfitService.selectClassType(inVar);
	...
	public ModelAndView selectClass(
		...
		// 대상업장 목록 조회
		RecordSet rs = themeItemProfitService.selectClass(parameter);
	...
	public ModelAndView selectItem(
		...
		// 자재별 목록 조회
		RecordSet rs = themeItemProfitService.selectItem(parameter);
	...
	public ModelAndView selectList(
		...
		//테마자재율 등록 을(를) 조회한다.
		RecordSet rs = themeItemProfitService.selectList(inVar);
	...
	/**
	 * 테마자재율 등록(을)를 Insert/Update/Delete 처리한다.
	 * <pre>
	 * - 메소드 상세설명 작성(생략 가능)-->생략 시 이 부분 삭제할 것
	 * </pre>
	 *
	...
	public ModelAndView saveList(
		...
		//테마자재율 등록을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = themeItemProfitService.saveList(parameter);
	...
	public ModelAndView saveClass(
		...
		int processCnt = themeItemProfitService.saveClass(parameter);
	...
	public ModelAndView deleteClass(
		...
		//이익율 설정을을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = themeItemProfitService.deleteClass(parameter);
	...
	/**
	 * 테마자재률 자재 (을)를 Insert/Update/Delete 처리한다.
	 * <pre>
	 * N Transaction
	 * </pre>
	 *
	...
	public ModelAndView saveItem(
		...
		try {
				//테마자재율(를) 등록, 수정, 삭제 처리한다.
				int processCnt = themeItemProfitService.saveItem(parameter);
				...
				if ("I".equals(saveFlag)) {
					deleteCnt = themeItemProfitService.deleteAll(parameter);
				}
	...
	public ModelAndView saveItem2(
		...
		//이익율 설정을을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = themeItemProfitService.saveItem2(parameter);
	...
	public ModelAndView deleteItem(
		...
		//자재를 등록, 수정, 삭제 처리한다.
		int processCnt = themeItemProfitService.deleteItem(parameter);
