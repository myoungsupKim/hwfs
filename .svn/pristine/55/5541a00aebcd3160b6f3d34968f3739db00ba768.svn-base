...
import com.hwfs.batch.fm.fms.dao.FmsMenupriceupdProDAO;
import com.hwfs.cmn.exception.BizException;
import com.hwfs.ls.cmn.util.LimsUtil;
import com.hwfs.sc.scb.service.BatchStepLogService;

/**
 * 식재영업 식단작성 단가 재 생성 Scheduler Job Class
 ...
 * </pre>
 */
public class FmsMenupriceupdProTasklet implements Tasklet {

    @Resource(name = "/batch/sc/fms/fmsMenupriceupdProDAO")
    private FmsMenupriceupdProDAO fmsMenupriceupdProDAO;
    ...
    public RepeatStatus execute(StepContribution step, ChunkContext chunkContext) throws Exception {
        ...
        procParam.put("p_date", LimsUtil.getDateAdd("yyyyMMdd", "date", 1));
        procParam.put("p_upjang", "");
        retProcVal = fmsMenupriceupdProDAO.fmsMenupriceupdPro(procParam);

        // 로그를 쌓는다.
        String strRtnMsg = "";
        if (!LimsUtil.checkNull(retProcVal.get("O_RTN")).equals("TRUE")) {
            strRtnMsg = LimsUtil.checkNull(retProcVal.get("O_RTNMSG"));
            batchStepLogService.insertNewTx(stepExecutionId, "FmsMenupriceupdProStep", "FAILED", strRtnMsg);
        ...
        batchStepLogService.insertNewTx(stepExecutionId, "FmsMenupriceupdProStep", "OK", strRtnMsg);
