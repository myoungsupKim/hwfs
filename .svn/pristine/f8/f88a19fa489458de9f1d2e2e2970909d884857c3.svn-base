 ...
 * </pre>
 */
public class FmsCustSysInterfaceSISSubinvTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution step, ChunkContext chunkContext) throws Exception {
    	...
    	System.out.println("start batch execute");
        ...
        procCount = fmsCustSysInterfaceSISDAO.batchJobSubinv(param);
        
        String strRtnMsg = "";
        strRtnMsg = "[" + procCount + "] 건이 처리 되었습니다..";
        
        if (  procCount == -1 ) {
            batchStepLogService.insertNewTx(stepExecutionId, "FmsCustSysInterfaceSISSubinvStep", "FAILED", strRtnMsg);
            step.setExitStatus(ExitStatus.FAILED);
            throw new BizException (strRtnMsg);
        }
        
        batchStepLogService.insertNewTx(stepExecutionId, "FmsCustSysInterfaceSISSubinvStep", "OK", strRtnMsg);
