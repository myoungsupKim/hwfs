...
/**
 * 식재사업부 채권선수금 자동대체 Scheduler Job Class
 ...
 * </pre>
 */
public class FmsArAdJobProTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution step, ChunkContext chunkContext) throws Exception {
        ...
        procParam.put("P_OCCUR_DATE", LimsUtil.getDate("yyyyMMdd"));
        procParam.put("P_MU_CD", "2004");
        procParam.put("P_USER", "SYSTEM-BT");
        retProcVal = fmsJobScheduleDAO.fmsArAdJobPro(procParam);

        // 로그를 쌓는다.
        String strRtnMsg = "";
        if (!LimsUtil.checkNull(retProcVal.get("O_RTN")).equals("TRUE")) {
            strRtnMsg = LimsUtil.checkNull(retProcVal.get("O_RTNMSG"));
            batchStepLogService.insertNewTx(stepExecutionId, "FmsArAdJobProStep", "FAILED", strRtnMsg);
        ...
        batchStepLogService.insertNewTx(stepExecutionId, "FmsArAdJobProStep", "OK", strRtnMsg);
