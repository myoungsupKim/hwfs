...
<%
	try	{
		...
		String strChk       = nullToBlank(in_vl.getString("strChk"));
		...
		sbSql.append("				    , TO_CHAR(A.PAY_TIME,'YYYY-MM-DD HH24:MI') AS PAY_TIME \n");		
		...
		sbSql.append("  			    , A.SEND_NAME, SC_CRYPTO_FUN('DEC', A.SEND_TEL_ENC,'4') SEND_TEL, A.SEND_POSTNO, SC_CRYPTO_FUN('DEC', A.SEND_ADDR_ENC,'6') SEND_ADDR \n");
		sbSql.append("  			    , A.RECE_NAME, SC_CRYPTO_FUN('DEC', A.RECE_TEL_ENC,'4') RECE_TEL, SC_CRYPTO_FUN('DEC', A.RECE_HP_ENC,'4') RECE_HP, A.RECE_POSTNO, SC_CRYPTO_FUN('DEC', A.RECE_ADDR_ENC,'6') RECE_ADDR \n");
...
//		sbSql.append("     				, (SELECT Z.CODE_NAME FROM HLDC_PO_CODE Z WHERE Z.GROUP_CODE = '025' AND Z.CODE = A.DELI_COMP) AS DELI_COMP \n");
		...
		//sbSql.append("     				, (B.STOCK_QTY - NVL((SELECT SUM(Z.PR_QTY) FROM HLDC_PO_SPECIAL_SALE Z WHERE Z.ITEM_CODE = B.ITEM_CODE AND Z.SP_TYPE = 'ING'),0)) AS STOCK_QTY \n");
...
//		sbSql.append("   AND A.LINE_STATUS = '정산'  \n");
		sbSql.append("   AND A.LINE_STATUS IN ('정산','배송') \n");
		if (strChk.equals("0"))
			sbSql.append("   AND A.INVOICE_NUM IS NULL \n");
		sbSql.append(" ORDER BY A.SP_DATE, A.ITEM_CODE \n");
