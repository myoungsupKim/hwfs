...
<%
	try	{
		...
		sbSql.append("     , SUM(A.TOTAL) AS TOTAL \n");
		sbSql.append("     , SUM(A.TOTALDC) AS TOTALDC \n");
		if (nullToBlank(ds_Cond.getString(0, "LOC")).equals(""))
			sbSql.append("     , DECODE(SUM(A.TOTAL),0,0,(SUM(A.TOTAL)-SUM(A.TOTALDC))/SUM(A.TOTAL)*100) AS DCRATE \n");
		else
			sbSql.append("     , SUM(A.DCRATE) AS DCRATE \n");
		sbSql.append("     , SUM(A.NET) AS NET \n");
		sbSql.append("     , SUM(A.GROSS) AS GROSS \n");
		if (nullToBlank(ds_Cond.getString(0, "LOC")).equals(""))
			sbSql.append("     , NULL AS DCNM \n");
		else
			sbSql.append("     , A.DCCODE||' '||A.DCNM AS DCNM \n");
		sbSql.append("  FROM FMB_PLAZA_DAILY_DC A \n");
		...
		if (! nullToBlank(ds_Cond.getString(0, "LOC")).equals(""))
			sbSql.append("        , A.LOC, A.BILLNO, A.DCCODE, A.DCNM \n");
		sbSql.append(" ORDER BY A.SDATE \n");
		if (! nullToBlank(ds_Cond.getString(0, "LOC")).equals(""))
			sbSql.append("        , A.LOC, A.BILLNO \n");
