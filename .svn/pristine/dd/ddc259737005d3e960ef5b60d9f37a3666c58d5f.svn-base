...
<%
	try	{
		...
		//입력 파라메터
		String sFavorName   = nullToBlank(in_vl.getString("FAVOR_NAME"));
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주 쿼리 변수 바인딩기능 추가			
            ...
            sbSql_tmp.append("/* fm/fmp/FMP00781V_T001.jsp */                                                 \n");
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주
		...
		if (sOtcustcd.equals(""))
		{			
			sbSql.append( "SELECT M.FAVOR_GROUP \n");
			sbSql.append( "     , N.FAVOR_NAME \n");
			sbSql.append( "     , T.ITEM_CODE \n");
			sbSql.append( "     , T.ITEM_NAME \n");
			sbSql.append( "     , T.ITEM_SIZE \n");
			sbSql.append( "     , T.PO_UOM, TAX_CODE \n");
			sbSql.append( "     , ROUND(SUM(NVL(T.IN_PRICE,0)) / DECODE(SIGN(SUM(NVL(T.PR_QTY,0))), -1,SUM(NVL(T.PR_QTY,0)), 0, 1, 1,SUM(NVL(T.PR_QTY,0))),2) AS SALE_PRICE  \n");
		...
		if (sOtcustcd.equals(""))		
//			sbSql.append( ") GROUP BY ITEM_CODE, ITEM_NAME, ITEM_SIZE, PO_UOM, TAX_CODE \n");		
    		sbSql.append( ") T, FMP_FAVORITE M, FMP_FAVORITE_GROUP N WHERE T.UPJANG = M.UPJANG  AND T.ITEM_CODE = M.ITEM_CODE   AND M.UPJANG = N.UPJANG   AND M.FAVOR_GROUP = N.FAVOR_GROUP GROUP BY T.ITEM_CODE, T.ITEM_NAME, T.ITEM_SIZE, T.PO_UOM, T.TAX_CODE,M.FAVOR_GROUP, N.FAVOR_NAME\n");			
