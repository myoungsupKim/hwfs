...
import com.hwfs.sm.stm.service.TaxInvoiceMgntHqService;


 /**
 * 세금계산서 발행을 관리하는 TaxInvoiceMgntHq Controller
 * 
 * @ClassName TaxInvoiceMgntHqController.java
 * @Description TaxInvoiceMgntHqController Class 
 ...
 */
@Controller
@RequestMapping("/sm/stm/taxInvoiceMgntHq/")
public class TaxInvoiceMgntHqController extends DefaultController {
	
	/** LogService */
//	private final Logger logger = LoggerFactory.getLogger (this.getClass());

	/** TaxInvoiceMgntHqService Bean 생성 */
	@Resource(name = "/sm/stm/taxInvoiceMgntHqService")
	private TaxInvoiceMgntHqService taxInvoiceMgntHqService;
	...
	public ModelAndView selectList(
		...
		RecordSet rs = taxInvoiceMgntHqService.selectList(parameter);
		
		//XPlatform으로 넘겨줄 Dataset을 설정한다.
		super.addOutDataSet(mav, "ds_list", rs);
		
		if(parameter.get("taxShetNum") != null && !parameter.get("taxShetNum").equals("")){
			RecordSet rsDtl = taxInvoiceMgntHqService.selectTaxShetDetailList(parameter);
	...
	public ModelAndView saveList(
		...
		DataSetMap taxShetOccurObj = param.get("ds_taxShetOccurObj");

		//세금계산서 발행(본사용)을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = taxInvoiceMgntHqService.saveTaxInvoice(taxShetMst, taxShetDtl, taxShetOccurObj);
	...
	public ModelAndView selectUpjangInfo(
		...
		//세금계산서 발행(본사용) 을(를) 조회한다.
		RecordSet rs = taxInvoiceMgntHqService.selectUpjangInfo(inVar);
	...
	public ModelAndView selectInchargeInfo(
		...
		//세금계산서 발행(본사용) 을(를) 조회한다.
		RecordSet rs = taxInvoiceMgntHqService.selectInchargeInfo(inVar);
	...
	public ModelAndView taxInvoiceRequest(
		...
		//RecordSet rs = taxInvoiceMgntHqService.taxInvoiceRequest(parameter);
		Map<String, Object> map = taxInvoiceMgntHqService.taxInvoiceRequest(parameter, reqStatus, loginDto);
	...
	public ModelAndView deleteTaxInvoice(
		...
		//세금계산서 발행(본사용)을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = taxInvoiceMgntHqService.deleteTaxInvoice(parameter);
	...
	public ModelAndView saveConcellation(
		...
		//세금계산서 발행(본사용)을(를) 등록, 수정, 삭제 처리한다.
		Map<String, String> returnValue = taxInvoiceMgntHqService.saveConcellation(taxShetMst, taxShetDtl);
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping("deleteTaxInvoiceOrgnl.xdo")
	public ModelAndView deleteTaxInvoiceOrgnl(
			@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		//세금계산서 발행(본사용)을(를) 등록, 수정, 삭제 처리한다.
		int processCnt = taxInvoiceMgntHqService.deleteTaxInvoiceOrgnl(parameter);
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping("selectInvoiceRcvr.xdo")
	public ModelAndView selectInvoiceRcvr(
			@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		//세금계산서 발행(본사용) 을(를) 조회한다.
		RecordSet rs = taxInvoiceMgntHqService.selectInvoiceRcvr(parameter);
