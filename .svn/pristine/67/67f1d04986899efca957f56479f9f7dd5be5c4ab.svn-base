...
<%
	...
	try
	{
		...
		for( int i = 0; i < ds_list.getRowCount(); i++ )
		{
			...
			sbSql.append("     , TO_CHAR(LAST_DAY(TO_DATE(TO_CHAR(ADD_MONTHS('" + PAY_TIME + "', -1), 'YYYYMM')||'01', 'YYYYMMDD')), 'YYYYMMDD') AS LASTDAY \n");
			sbSql.append("     , TO_CHAR(TO_DATE(TO_CHAR(ADD_MONTHS('" + PAY_TIME + "', -1), 'YYYYMM'), 'YYYYMM'), 'YYYY\"년\"MM\"월\"') AS STR_MONTH \n");
			sbSql.append("     , TO_CHAR(LAST_DAY(TO_DATE(TO_CHAR(ADD_MONTHS('" + PAY_TIME + "', -1), 'YYYYMM')||'01', 'YYYYMMDD')), 'YYYY\"년\"MM\"월\"DD\"일\"') AS STR_DAY \n");
			...
			for( int n = 0; n < ds_sp.getRowCount(); n++)
			{
				...
				sbSql.append("                  FROM (SELECT TO_CHAR(ADD_MONTHS(TO_DATE('" + PAY_TIME + "', 'YYYYMMDD'), -1), 'YYYYMM') MM FROM DUAL) A \n");
				...
				sbSql.append("               ) \n");
				sbSql.append("       ) X \n");
				sbSql.append(" WHERE X.CLOSE_DATE = TO_CHAR(LAST_DAY(TO_DATE(TO_CHAR(ADD_MONTHS(TO_DATE('" + PAY_TIME + "', 'YYYYMMDD'), -1), 'YYYYMM')||'01', 'YYYYMMDD')), 'YYYYMMDD') \n");
				...
				// 정산월 마지막일의 일마감이 처리됐으면 전기이월 불가
				if( closeDay_yn.equals("Y") )
				{
					if( closeMonth_yn.equals("Y") )
					{
						out_vl.add("v_spClose", UPJANGNM + " 사업장의 " + sDay +  " 매출 마감이 처리되어 매출 이월을 할 수 없습니다.");
					   	this.setResultMessage(0, "OK", out_vl);
					   	proc_output(response, out, out_vl, out_dl);
					   	return;
					}
				}
				// 정산월의 마지막일의 일마감이 처리 안됐으면 (-)매출일은 현재일, (+)매출일은 전월 마지막일이 된다. 
				else
				{
					payDay1 = toDay;
					payDay2 = lastDay;
				...
				sbSql.append("     , REMARK        = REMARK || ';' || LINE_STATUS || '=>매출전기이월:' || '" + g_EmpNo + "' \n");
				sbSql.append("     , RECEIPT_DATE  = TO_DATE('" + lastDay + "', 'YYYYMMDD') \n");
