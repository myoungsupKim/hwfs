...
<%
	try	{
		...
		String CLASS1G     = nullToBlank(in_vl.getString("CLASS1G"));
		String CLASS2G     = nullToBlank(in_vl.getString("CLASS2G"));
		String CLASS3G     = nullToBlank(in_vl.getString("CLASS3G"));
		String CLASS4G     = nullToBlank(in_vl.getString("CLASS4G"));
		String ITEM_CODEG  = nullToBlank(in_vl.getString("ITEM_CODEG"));
        ...
        if (UNIT.equals("1"))
        {
    	    ...
    	    sbSql.append("     , SUM(ROUND((A.ADJUST_QTY+A.PO_QTY)*A.SALE_PRICE)) AS REQ_AMT \n");
        	sbSql.append("     , SUM(A.DELIVERED_QTY) AS IN_QTY \n");
        	sbSql.append("     , SUM(ROUND(A.DELIVERED_QTY*A.SALE_PRICE)) AS IN_AMT \n");
        }
        if (UNIT.equals("0"))
        {
	        sbSql.append("     , SUM((A.ADJUST_QTY+A.PO_QTY)*C.UOM_CONVERT_RATE) AS REQ_QTY \n");
    	    sbSql.append("     , SUM(ROUND((A.ADJUST_QTY+A.PO_QTY)*A.SALE_PRICE)) AS REQ_AMT \n");
        	sbSql.append("     , SUM(A.DELIVERED_QTY*C.UOM_CONVERT_RATE) AS IN_QTY \n");
        	sbSql.append("     , SUM(ROUND(A.DELIVERED_QTY*A.SALE_PRICE)) AS IN_AMT \n");
        }
    	sbSql.append("     , B.CENTER_NAME||'-'||B.TM_NM||'-'||B.UPJANGNM PL_CD \n");
        ...
        if (TAB_INDEX.equals("Z"))
        {
	        sbSql.append("   AND C.ITEM_CLASS1 = '" + CLASS1G + "' \n");
        }
        if (TAB_INDEX.equals("A"))
        {
	        sbSql.append("   AND C.ITEM_CLASS2 = '" + CLASS2G + "' \n");
        }
        if (TAB_INDEX.equals("B"))
        {
	        sbSql.append("   AND C.ITEM_CLASS3 = '" + CLASS3G + "' \n");
        }
        if (TAB_INDEX.equals("C"))
        {
	        sbSql.append("   AND C.ITEM_CLASS4 = '" + CLASS4G + "' \n");
        }
        if (TAB_INDEX.equals("D"))
        {
	        sbSql.append("   AND C.ITEM_CODE = '" + ITEM_CODEG + "' \n");
        }
        sbSql.append(" GROUP BY B.CENTER_NAME \n");
        sbSql.append("        , B.SC_NM \n");
        sbSql.append("        , B.MU_NM \n");
        sbSql.append("        , B.TM_NM \n");
        sbSql.append("        , B.JOB_NAME \n");
