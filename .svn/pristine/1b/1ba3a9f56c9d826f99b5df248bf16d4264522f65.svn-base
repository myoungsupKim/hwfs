...
<%
	try	{
		...
		if (sType.equals("QryItm"))
		{
			...
			sbSql.append("     , CASE WHEN F.CONTRACT_PRICE IS NULL THEN B.UNIT_PRICE ELSE F.CONTRACT_PRICE END AS UNIT_PRICE \n");
	        ...
	        sbSql.append("   AND A.BU_CD       = '2000' \n");
         	...
         	sbSql.append("                    , SCO_UPJANG_MST_V SC \n");
         	sbSql.append("                WHERE HR.SABUN = A.RQ_SABUN \n");
         	sbSql.append("                  AND HR.CC_CD = SC.CC_CD \n");

         	if( !nullToBlank(ds_Cond.getString(0, "RQ_MU")).equals("") )
         		sbSql.append("                  AND SC.MU_CD = '" + nullToBlank(ds_Cond.getString(0, "RQ_MU")) + "' \n");

         	if( !nullToBlank(ds_Cond.getString(0, "RQ_TM")).equals("") )
         		sbSql.append("                  AND SC.TM_CD = '" + nullToBlank(ds_Cond.getString(0, "RQ_TM")) + "' \n");

         	if( !nullToBlank(ds_Cond.getString(0, "RQ_UPJANG")).equals("") )
         		sbSql.append("                  AND SC.UPJANG = '" + nullToBlank(ds_Cond.getString(0, "RQ_UPJANG")) + "' \n");
