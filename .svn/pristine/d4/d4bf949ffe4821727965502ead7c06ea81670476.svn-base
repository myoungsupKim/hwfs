...
<%
	...
	try {
		...
		sbSql.append("SELECT TM_CD, TM_NM \n");
		...
		sbSql.append("     , SUM(AMT01 + AMT02 + AMT03 + AMT04 + AMT05 + AMT06 + AMT07 + AMT08 + AMT09 + AMT10 + AMT11 + AMT12 + AMT13 + AMT14 + AMT15 + AMT16 + AMT17 + AMT18) AS AMT_TOT \n");
		sbSql.append("  FROM ( \n");
		sbSql.append("        SELECT D.TM_CD, D.TM_NM \n");
		...
		sbSql.append("             , ( \n");
		sbSql.append("                SELECT C.TM_CD, C.TM_NM, A.ITEM_CODE, D.ITEM_CLASS2 \n");
		...
		sbSql.append("                 WHERE A.TRANS_DATE BETWEEN '" + DATE + "' || '01' AND '" + DATE + "' || '31' -- [기간] \n");
		if (!TM_CD.equals("")) 
			sbSql.append("                    AND (C.TM_CD = '" + TM_CD  + "' OR C.PT_CD = '" + TM_CD + "')  --[운영부서] \n");
		if (!UPJANG.equals("")) 
		...
		sbSql.append("                 GROUP BY C.TM_CD, C.TM_NM, D.ITEM_CLASS2, A.ITEM_CODE \n");
		sbSql.append("                ) D \n");
		sbSql.append("         WHERE A.CLASS_CODE  = D.ITEM_CLASS2 \n");
		sbSql.append("         GROUP BY D.TM_CD, D.TM_NM, A.CLASS_CODE \n");
		sbSql.append("       ) \n");
		sbSql.append(" GROUP BY TM_CD, TM_NM \n");
