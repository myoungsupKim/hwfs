...
<%
    try	{
        ...
        // PB상품 only
        String pbOnly 		  = nullToBlank(in_vl.getString("pbOnly"));
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주 쿼리 변수 바인딩기능 추가
            ...
            sbSql_tmp.append("/* fm/fmp/FMP00080E_T001.jsp */                                                 \n");
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주
        ...
        */
        ...
        sbSql.append("SELECT /* [2018031601 임시] 아이템 검색-FMP00080E_T001_renewbind.jsp*/ \n");
...
// 		sbSql.append("                    , 0 ) + NVL(K.D_DAYS,0) + 1 AS D_DAYS \n");
        ...
        // PB상품 ONLY
         if (pbOnly.equals("TRUE")) {
        	 sbSql.append("  WHERE M.FEATURE_NAME = 'PB상품'  \n"); 
         }
        
        
        if (strIndex.equals("1")){
            sbSql.append(" ORDER BY DECODE(M.FEATURE_NAME, 'PB상품', '1', ''), DECODE(M.FEATURE_NAME, '기획자재', '1', ''), NVL(M.TOT_SO_AMT, 0) DESC, M.ITEM_CODE \n");
        } else if (strIndex.equals("2")){
