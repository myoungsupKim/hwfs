...
<%
	...
	try {
		...
		String DIR_FROM      = ServiceManagerFactory.getProperty("global.imagerepository.path") + "/ms";  // 모바일 루트
		...
		for ( int i = 0 ; i < RowCnt ; i++ )
		{
			...
			//System.out.println("FROM : " + DIR_FROM + PRE_VIEW);
			//System.out.println("TO : " + DIR_TO + "/" + ORGNL_IMGE);
			
			// 파일 이동처리
			FileUtils.copyFile(new File(DIR_FROM + PRE_VIEW), new File(DIR_TO + "/" + ORGNL_IMGE));
			
			sbSql.delete(0, sbSql.length());
			sbSql.append("INSERT INTO FSC_CONTRT_CHK_IMGE ( \n");
			sbSql.append("       SRLNUM                     \n");			
			...
			sbSql.append(") VALUES (                        \n");
			sbSql.append("       (SELECT NVL(MAX(SRLNUM), 0) + 1 FROM FSC_CONTRT_CHK_IMGE) \n");
			sbSql.append("     , '" + TABLE_TYPE       + "' \n");
			sbSql.append("     , '" + FORM_CD          + "' \n");
			sbSql.append("     , '" + CUSTCD           + "' \n");
			sbSql.append("     , '" + CHK_DD           + "' \n");
