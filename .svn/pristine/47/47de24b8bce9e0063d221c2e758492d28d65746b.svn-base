...
<%
    try	{
        ...
        String strTabIndex      = nullToBlank(in_vl.getString("strTabIndex"));
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주 쿼리 변수 바인딩기능 추가
            ...
            sbSql_tmp.append("/* fm/fmp/FMP00082E_T002.jsp */                                                 \n");
...
// [속도개선 Project 시작] 2017. 8. 28. 최범주
        ...
        */
        ...
        sbSql.append("		FROM ( \n");
        sbSql.append("SELECT /* [2018031601 임시] 아이템 검색-FMP00082E_T002_renewbind.jsp*/ \n");
        //sbSql.append("       T.* ");
        sbSql.append("       T.ITEM_CODE, T.ITEM_NAME, T.ITEM_SIZE, T.PO_UOM, T.UNIT_PRICE, T.MARGIN_PRICE, T.SALE_PRICE, T.PR_QTY, T.LINE_STATUS \n");
        sbSql.append("       , T.CLS_STATUS, T.PR_REMARK, T.SUBINV_CODE, T.NEED_DATE, T.PRE_NEED_DATE, T.TAX_CODE, T.CUSTCD, T.CENTER_FLAG, T.CENTER_CODE \n");
        sbSql.append("       , T.POINT_FLAG, T.PR_UPJANG, T.RC_UPJANG, T.INVAT_FLAG, T.OUTVAT_FLAG, T.MIN_ORD_QTY, T.MAX_ORD_QTY, T.MULTIPLIER_QTY, T.OTCUSTCD \n");
        sbSql.append("       , T.OP_RATE, T.D_DAYS, T.D_TIMES, T.D_DAY_DISP, T.OTCUSTNM, T.KG_CONVERT_RATE, T.FEATURE_NAME_CHK, T.IMAGE_PATH, T.IMAGE_YN \n");
        sbSql.append("       , T.CONSUM_YN, T.PR_ID, T.KEEPING_TYPE, T.APPLY_SD, T.VD_SN, T.ORIGIN_TYPE, T.MOBILE_GUBUN, T.UPJANG_GRP, T.PO_STATUS, T.UNUSE_YN \n");
        sbSql.append("       , T.VAL_STP_YN, T.FMP_STATUS, T.PO_WEEK_STATUS, T.DANGA_ITEM_CODE, T.UOM_CONVERT_RATE, T.TOT_WGHT, T.FAVOR_GROUP_FLAG, T.WIN_ITEM_CD \n");
        sbSql.append("       , ROUND(T.SALE_PRICE * T.PR_QTY) AS SALE_AMOUNT");
        
        
        
        sbSql.append("       , SCC_SOPR_AGG_FUN( ? , T.ITEM_CODE, '1') AS MAX_SO_QTY \n");
        sbSql.append("       , SCC_SOPR_AGG_FUN( ? , T.ITEM_CODE, '3') AS TOT_SO_AMT \n");
        ...
        sbSql.append("       , DECODE(T.FEATURE_NAME_CHK, NULL, ( CASE WHEN T.EVENT_ID IS NOT NULL THEN '특가행사' ELSE U.FEATURE_NAME END ) , T.FEATURE_NAME_CHK||U.FEATURE_NAME) AS FEATURE_NAME \n");
        
        //sbSql.append("     , U.FEATURE_ICON ");
        sbSql.append("       , CASE WHEN T.EVENT_ID IS NOT NULL THEN 'theme://Images/new/grd_WF_Icon06.png' ELSE U.FEATURE_ICON END AS FEATURE_ICON \n");
        ...
        // 수정필요 
        //sbSql.append("     , ROUND(A.SALE_PRICE * B.DEF_QTY) AS SALE_AMOUNT \n");
...
/*         sbSql.append("     , CASE WHEN CC.OP_PRICE IS NOT NULL OR CC.OP_RATE IS NOT NULL \n");
        ...
        sbSql.append("            WHEN (DD.OP_PRICE IS NOT NULL OR DD.OP_RATE IS NOT NULL) AND A.CENTER_CODE <> '0' \n");
...
// 공통처리5:30
        sbSql.append("            , DECODE( SIGN(TO_DATE( ? ,'YYYYMMDD') - TO_DATE(TO_CHAR(SYSDATE,'YYYYMMDD'),'YYYYMMDD') - NVL(K.D_DAYS,0) ) \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        sbSql.append("                    , 0, DECODE(SIGN(DECODE(  \n");
        sbSql.append(" 												(SELECT CASE WHEN Z.SET1 <= ? AND Z.SET2 >= ? THEN ATTR01 ELSE ATTR02 END FROM SCC_COMMON_CODE Z WHERE GROUP_CODE = 'SYS_PROPERTY' AND CODE = 'SO_PR_RTN') \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));        
        sbSql.append("		 ,'',24,0,24, \n");
        sbSql.append(" 					(SELECT CASE WHEN Z.SET1 <= ? AND Z.SET2 >= ? THEN ATTR01 ELSE ATTR02 END FROM SCC_COMMON_CODE Z WHERE GROUP_CODE = 'SYS_PROPERTY' AND CODE = 'SO_PR_RTN') \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));        
        sbSql.append("		 ) - TO_NUMBER(TO_CHAR(SYSDATE,'HH24'))),1,0,1) \n");
        sbSql.append("                    , 0 ) + NVL(K.D_DAYS,0) AS D_DAYS \n");
        sbSql.append("            , DECODE( \n");
        sbSql.append(" 						(SELECT CASE WHEN Z.SET1 <= ? AND Z.SET2 >= ? THEN ATTR01 ELSE ATTR02 END FROM SCC_COMMON_CODE Z WHERE GROUP_CODE = 'SYS_PROPERTY' AND CODE = 'SO_PR_RTN') \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        sbSql.append("		,'',2400,0,2400, \n");
        sbSql.append(" 						(SELECT CASE WHEN Z.SET1 <= ? AND Z.SET2 >= ? THEN ATTR01 ELSE ATTR02 END FROM SCC_COMMON_CODE Z WHERE GROUP_CODE = 'SYS_PROPERTY' AND CODE = 'SO_PR_RTN') \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        sbSql.append("		) AS D_TIMES \n");
        ...
        if (strOnly.equals("TRUE"))
        {
            ...
            if (sType.equals("DC")){            
            	sbSql.append("			 AND  A.CENTER_FLAG = 'DC' \n");
            }
        ...
        //반환발주의 경우
        if (strIndex.equals("1")){            
        	sbSql.append("		AND EXISTS (SELECT 1 FROM RTN_ITEM_MST X WHERE X.ITEM_CODE = A.ITEM_CODE AND X.USE_YN = 'Y') \n");
        }
...
//5:30
        ...
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        sbSql.append("		,'',2400,0,2400,TO_NUMBER( \n");
        sbSql.append(" 									(SELECT CASE WHEN Z.SET1 <= ? AND Z.SET2 >= ? THEN ATTR01 ELSE ATTR02 END FROM SCC_COMMON_CODE Z WHERE GROUP_CODE = 'SYS_PROPERTY' AND CODE = 'SO_PR_RTN') \n");
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        bindString[iBind++] = nullToBlank(ds_cond.getString(0, "NEED_DATE"));
        sbSql.append("				)) - TO_NUMBER(TO_CHAR(SYSDATE,'HH24MI'))),1,0,1) \n");
...
//                sbSql.append("   	OR  A.ITEM_SIZE LIKE '%'||?||'%' \n");
      ...
      //전자구매 수발주제어 예외관리 추가 2022.05.06
      //전자구매 수발주제어 예외관리 로직 제외요청 - 주석처리 2022.09.07
        /*
        if (strOnly.equals("TRUE"))
        {
        	//반환이 아닌 경우 (교환, 증정..)
           // if (!strTabIndex.equals("0")){ //고객사용 해지   
        	sbSql.append("   WHERE 1=1 \n");
        ...
        */
        ...
        //if (strIndex.equals("1")){
            sbSql.append(" ORDER BY DECODE(M.FEATURE_NAME, '기획자재', '1', ''), NVL(M.TOT_SO_AMT, 0) DESC, M.ITEM_CODE \n");
        /*} else if (strIndex.equals("2")){            
