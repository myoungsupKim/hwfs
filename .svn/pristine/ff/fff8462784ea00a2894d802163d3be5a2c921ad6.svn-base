...
<%
	try	{
		...
		if (sOtCust.equals("0000") || sOtCust.equals(""))
		{
			...
			sbSql.append("  FROM (SELECT A.* , DECODE(SUBSTR(A.TRANS_DATE,1,6), SUBSTR(A.MG_DATE,1,6), A.TRANS_DATE, MG_DATE) AS AA_DATE, B.ITEM_CLASS4, B.ORIGIN_TYPE FROM FMS_TRANSACTION_V A, FMS_ITEM_V B WHERE A.ITEM_CODE = B.ITEM_CODE) A, FMS_SUBINVENTORY Z \n");
		...
		if ((! sOtCust.equals("0000")) || sOtCust.equals(""))
		{
			...
			sbSql.append("     , ROUND(DECODE(A.TAX_CODE,'100',1.1,1) * A.OP_AMOUNT,0) AS OP_TOT \n");
			sbSql.append("  FROM (SELECT A.* , B.ITEM_CLASS4, B.ORIGIN_TYPE FROM FMP_OTCUST_PR A, FMS_ITEM_V B WHERE A.ITEM_CODE = B.ITEM_CODE) A, FMS_SUBINVENTORY Z \n");
			sbSql.append(" WHERE A.SUBINV_CODE = Z.SUBINV_CODE \n");
