	...
	try {
		...
		if (sourceItemCnt > 1) {
			for(int i=0; i<sourceItemCnt; i++) {
				...
				sbSql.append("   AND UPJANG IN (SELECT UPJANG FROM HLDC_PO_UPJANG_CENTER WHERE CENTER_CODE = ?) \n");
				...
				pstmt.setString(idx++, CENTER_CODE);
				pstmt.setString(idx++, TARGET_ITEM_CODE);
				pstmt.setString(idx++, CENTER_CODE);
		...
		// 변경 전 자재가 1개일 경우 선택된 레시피를 대상으로 변경처리 한다.
			...
			for(int i=0; i<recipeCnt; i++) {
				String UPJANG = nullToBlank(ds_recipeList.getString(i, "UPJANG"));
