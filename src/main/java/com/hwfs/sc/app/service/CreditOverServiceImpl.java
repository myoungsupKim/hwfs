...
import com.hwfs.sc.app.dao.CreditOverDAO;

 /**
 * 식재앱 임시여신 인터페이스용  ServiceImpl에 대한 설명 작성
 * @ClassName CreditOverServiceImpl.java
 * @Description CreditOverServiceImpl Class 
 ...
 */
@Service
public class CreditOverServiceImpl extends DefaultServiceImpl implements CreditOverService {
	private final Logger logger = LoggerFactory.getLogger (this.getClass());
	
	@Resource(name="/sc/app/creditOverDAO")
	private CreditOverDAO creditOverDAO;
	...
	*/
	...
	@CustomAnnotationDefinition(txId="IF_CREDIT_OVER_SERVICE",Description="식재앱 임시여신 인터페이스용 Service")
	public JSONObject callService(JSONObject txHeader, JSONObject txData,JSONObject txResult) throws Exception {
		...
		if(txDiv.equals("SelectCreditOverHist")){
			...
			if(rs == null) {
				...
				if(rs.size() == 0)
				{
					...
					procResult.put("resultCount", 0);
					procResult.put("ErrorCode", -1); //오류
					procResult.put("ErrorMsg", "조회기간 동안 신청한 이력이 없습니다.");
				}
				else
				{
		...
		//임시여신 신청
			...
			try {
				...
				if(rs1.size() > 0){
					throw new Exception("미승인건이 존재합니다.");
				}
		...
		insLogCnt += creditOverDAO.logInsert(parameter);
		//로그저장end
