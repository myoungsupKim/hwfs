...
import com.hwfs.batch.ma.map.dao.MapDayActjobProDAO;
 ...
 * </pre>
 */
public class MapDayactjobProJobTasklet implements Tasklet {

    @Resource(name = "/batch/sc/map/MapDayActjobProDAO")
    private MapDayActjobProDAO mapDayactjobProDAO;
    ...
    public RepeatStatus execute(StepContribution step, ChunkContext chunkContext) throws Exception {
        ...
        retProcVal = mapDayactjobProDAO.mapDayactjobPro(procParam);

        // 로그를 쌓는다.
        String strRtnMsg = LimsUtil.checkNull(retProcVal.get("O_RTNMSG"));
        if (!LimsUtil.checkNull(retProcVal.get("O_RTN")).equals("TRUE")) {
            batchStepLogService.insertNewTx(stepExecutionId, "MapDayactjobProStep", "FAILED", strRtnMsg);
        }
        else
        {
            batchStepLogService.insertNewTx(stepExecutionId, "MapDayactjobProStep", "OK", strRtnMsg);
        }
