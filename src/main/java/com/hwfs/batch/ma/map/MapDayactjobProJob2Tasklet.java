...
import com.hwfs.batch.ma.map.dao.MapDayActjobPro2DAO;
import com.hwfs.ls.cmn.util.LimsUtil;
import com.hwfs.sc.scb.service.BatchStepLogService;

/**
 * 매출데이터 강제생성 Scheduler Job Class
 * @ClassName MapDayactjobProJob2Tasklet.java
 * @Description MapDayactjobProJob2Tasklet Class
 ...
 * </pre>
 */
public class MapDayactjobProJob2Tasklet implements Tasklet {

    @Resource(name = "/batch/sc/map/MapDayActjobPro2DAO")
    private MapDayActjobPro2DAO mapDayactjobPro2DAO;
    ...
    public RepeatStatus execute(StepContribution step, ChunkContext chunkContext) throws Exception {
        ...
        retProcVal = mapDayactjobPro2DAO.mapDayactjobPro2(procParam);

        // 로그를 쌓는다.
        String strRtnMsg = LimsUtil.checkNull(retProcVal.get("O_RTNMSG"));
        if (!LimsUtil.checkNull(retProcVal.get("O_RTN")).equals("TRUE")) {
            batchStepLogService.insertNewTx(stepExecutionId, "MapDayactjobPro2Step", "FAILED", strRtnMsg);
        }
        else
        {
            batchStepLogService.insertNewTx(stepExecutionId, "MapDayactjobPro2Step", "OK", strRtnMsg);
        }
