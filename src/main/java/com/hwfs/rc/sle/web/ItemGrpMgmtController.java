...
import com.hwfs.rc.sle.service.ItemGrpMgmtService;

 /**
 * ItemGrpMgmtController에 대한 설명 작성
 * @ClassName ItemGrpMgmtController.java
 * @Description ItemGrpMgmtController Class 
 ...
 *  2015. 3. 13.   박대영        최초생성
 * </pre>
 * @author FC종합전산구축 : 팀명 외식영업(POS영업관리)
 * @since 2015. 3. 13.
 * @version 1.0
 * @see 
 * <pre>
 ...
 */
@Controller
@RequestMapping("/rc/sle/itemGrpMgmt/")
public class ItemGrpMgmtController extends DefaultController {
	@Resource(name="/rc/sle/itemGrpMgmtService")
	private ItemGrpMgmtService itemGrpMgmt;
	...
	public ModelAndView selectList(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		RecordSet rs = itemGrpMgmt.selectList(parameter);
	...
	public ModelAndView searchList(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		RecordSet rs = itemGrpMgmt.searchList(parameter);
		
		//개인정보보호 접근로그 조회 조건과 건수
		//accessLogService.readLog(parameter, rs.size());
				
		//XPlatform으로 넘겨줄 Dataset을 설정한다.
		super.addOutDataSet(mav, "ds_menuClsfi", rs);
	...
	public ModelAndView selectListCheck(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		RecordSet rs = itemGrpMgmt.selectListCheck(parameter);
		
		//개인정보보호 접근로그 조회 조건과 건수
		//accessLogService.readLog(parameter, rs.size());
				
		//XPlatform으로 넘겨줄 Dataset을 설정한다.
		super.addOutDataSet(mav, "ds_menuClsfiSearch", rs);
	...
	public ModelAndView saveData(@Bind(id = "", type = XplatformMapDTO.class) XplatformMapDTO xpDTO) throws Exception {
		...
		int procCnt =  itemGrpMgmt.saveData(parameter.get("ds_menuClsfi"));
